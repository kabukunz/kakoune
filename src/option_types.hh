...
inline void option_from_string(StringView str, int &opt)
{
    opt = str_to_int(str);
}
...
inline void option_from_string(StringView str, size_t &opt)
{
    opt = str_to_int(str);
}
...
String option_to_string(const Vector<T, domain> &opt)
{
    ...
    for (size_t i = 0; i < opt.size(); ++i)
    {
        ...
        if (i != opt.size() - 1) res += list_separator;
    }
    return res;
}
...
bool option_add(Vector<T, domain> &opt, StringView str)
{
    ...
    std::copy(std::make_move_iterator(vec.begin()),
              std::make_move_iterator(vec.end()), back_inserter(opt));
...
String option_to_string(const IdMap<Value, domain> &opt)
{
    ...
    {
        if (it != begin(opt)) res += list_separator;
...
template <size_t I, typename... Types> struct TupleOptionDetail
{
    static String to_string(const std::tuple<Types...> &opt)
    {
        return TupleOptionDetail<I - 1, Types...>::to_string(opt) +
               tuple_separator + escape(option_to_string(std::get<I>(opt)),
                                        tuple_separator, '\\');
    }

    static void from_string(ConstArrayView<String> elems,
                            std::tuple<Types...> &opt)
    {
...
template <typename... Types> struct TupleOptionDetail<0, Types...>
{
    static String to_string(const std::tuple<Types...> &opt)
    {
...
template <typename RealType, typename ValueType>
inline void option_from_string(StringView str,
                               StronglyTypedNumber<RealType, ValueType> &opt)
{
...
template <typename T> bool option_add(T &, StringView str)
{
    throw runtime_error("no add operation supported for this option type");
}

template <typename EffectiveType, typename LineType, typename ColumnType>
inline void option_from_string(
    StringView str, LineAndColumn<EffectiveType, LineType, ColumnType> &opt)
{
    auto vals = split(str, ',');
    if (vals.size() != 2) throw runtime_error("expected <line>,<column>");
...
template <> struct WithBitOps<DebugFlags> : std::true_type
{
};

constexpr Array<EnumDesc<DebugFlags>, 3> enum_desc(DebugFlags)
{
    return {{{DebugFlags::Hooks, "hooks"},
...
inline bool operator==(const PrefixedList<P, T> &lhs,
                       const PrefixedList<P, T> &rhs)
{
...
inline void option_from_string(StringView str, PrefixedList<P, T> &opt)
{
    ...
    if (it != str.end()) option_from_string({it + 1, str.end()}, opt.list);
}
