...
Buffer *create_fifo_buffer(String name, int fd, bool scroll)
{
    ...
    std::unique_ptr<FDWatcher, decltype(watcher_deleter)> watcher(
        new FDWatcher(
            fd,
            [buffer, scroll, fifo_watcher_id](FDWatcher &watcher,
                                              EventMode mode) {
                if (mode != EventMode::Normal) return;
                ...
                // the
                // iteration number allows us to go back go back to the event
                // loop and
                ...
                do
                {
                    ...
                    const bool prevent_scrolling =
                        pos == ByteCoord{0, 0} and not scroll;
                    if (prevent_scrolling) pos = buffer->next(pos);
...
void write_to_debug_buffer(StringView str)
{
    ...
    else
    {
        ...
        BufferManager::instance().create_buffer(
            debug_buffer_name.str(),
            Buffer::Flags::NoUndo | Buffer::Flags::Debug, line, InvalidTime);
    }
}
}
