...
void write_buffer_to_fd(Buffer& buffer, int fd) {
  ...
  if (buffer.options()["BOM"].get<ByteOrderMark>() == ByteOrderMark::Utf8)
    if (::write(fd, "\xEF\xBB\xBF", 3) < 0)
      throw runtime_error(
          format("unable to write data to the buffer (fd: {}; errno: {})", fd,
                 ::strerror(errno)));
  ...
  while (it != end) {
    ...
    if (stat(dirname.zstr(), &st) == 0) {
      if (not S_ISDIR(st.st_mode))
        throw runtime_error(
            format("Cannot make directory, '{}' exists but is not a directory",
                   dirname));
    } else {
      ...
      if (mkdir(dirname.zstr(), S_IRWXU | S_IRWXG | S_IRWXO) != 0)
        throw runtime_error(
            format("mkdir failed for directory '{}' errno {}", dirname, errno));
    }
  }
}
...
CandidateList complete_filename(StringView prefix,
  ...
  auto filter = [&ignored_regex, check_ignored_regex, include_hidden, only_dir](
      const dirent& entry, struct stat& st) {
