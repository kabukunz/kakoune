...
{
    auto it = m_aliases.find(alias);
    if (it != m_aliases.end()) m_aliases.erase(it);
}
...
Vector<StringView> AliasRegistry::aliases_for(StringView command) const
{
    ...
    if (m_parent) res = m_parent->aliases_for(command);

    for (auto &alias : m_aliases)
    {
        if (alias.value == command) res.push_back(alias.key);
    }
...
Vector<std::pair<StringView, StringView>> AliasRegistry::flatten_aliases() const
{
    ...
    if (m_parent) res = m_parent->flatten_aliases();
    for (auto &alias : m_aliases)
    {
        if (not contains(
                res | transform([](const AliasDesc &val) { return val.first; }),
                alias.key))
