...
static LineCount adapt_view_pos(LineCount line,
                                LineCount offset,
                                LineCount view_pos,
                                LineCount view_size,
      ...
      if (atom.has_buffer_range()) {
        if (atom.begin() <= pos and atom.end() > pos) {
          ...
          if (atom.type() == DisplayAtom::BufferRange) {
            ...
            pos_beg =
                buffer_column + utf8::distance(buf.iterator_at(atom.begin()),
                                               buf.iterator_at(pos));
            pos_end = pos_beg + 1;
          } else {
...
namespace {
CharCount find_display_column(const DisplayLine& line,
                              const Buffer& buffer,
                              ByteCoord coord) {
  CharCount column = 0;
  for (auto& atom : line) {
...
ByteCoord find_buffer_coord(const DisplayLine& line,
                            const Buffer& buffer,
                            CharCount column) {
  ...
  for (auto& atom : line) {
    ...
    if (atom.has_buffer_range() and column < len) {
      if (atom.type() == DisplayAtom::BufferRange)
        return buffer.clamp(utf8::advance(buffer.iterator_at(atom.begin()),
...
ByteCoord Window::buffer_coord(CharCoord coord) const {
  ...
  return find_buffer_coord(m_display_buffer.lines()[(int)coord.line], buffer(),
                           coord.column);
}
...
ByteCoordAndTarget Window::offset_coord(ByteCoordAndTarget coord,
  ...
  m_builtin_highlighters.highlight(
      input_handler.context(), HighlightFlags::MoveOnly, display_buffer, range);
