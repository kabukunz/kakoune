...
class StaticRegister : public Register
{
   public:
    ...
    {
        m_content = Vector<String, MemoryDomain::Registers>(values.begin(),
                                                            values.end());
        return *this;
    }
...
template <typename Func> class DynamicRegister : public StaticRegister
{
   public:
    DynamicRegister(Func function) : m_function(std::move(function)) {}
    Register &operator=(ConstArrayView<String> values) override
    {
...
template <typename Func> std::unique_ptr<Register> make_dyn_reg(Func func)
{
...
class RegisterManager : public Singleton<RegisterManager>
{
   ...
   protected:
    UnorderedMap<char, std::unique_ptr<Register>, MemoryDomain::Registers>
        m_registers;
};
}
