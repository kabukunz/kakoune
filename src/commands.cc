...
const PerArgumentCommandCompleter filename_completer({[](const Context& context,
                                                         CompletionFlags flags,
                                                         const String& prefix,
                                                         ByteCount cursor_pos) {
  return Completions{
      0_byte, cursor_pos,
      complete_filename(prefix, context.options()["ignored_files"].get<Regex>(),
                        cursor_pos)};
}});
...
const PerArgumentCommandCompleter buffer_completer({[](const Context& context,
                                                       CompletionFlags flags,
                                                       const String& prefix,
                                                       ByteCount cursor_pos) {
...
void edit(const ParametersParser& parser,
          Context& context,
          const ShellContext&) {
...
const CommandDesc edit_cmd = {"edit", "e",
                              "edit [<switches>] <filename> [<line> "
                              "[<column>]]: open the given filename in a "
                              "buffer",
...
void write_buffer(const ParametersParser& parser,
                  Context& context,
                  const ShellContext&) {
...
const CommandDesc force_kill_cmd = {
    "kill!", nullptr,
    "kill current session, quit all clients and server, do not check for "
    "unsaved buffers",
    no_params, CommandFlags::None, CommandHelper{}, CommandCompleter{},
    [](const ParametersParser&, Context&, const ShellContext&) {
      throw kill_session{};
...
template <bool force>
void write_quit(const ParametersParser& parser,
                Context& context,
...
template <bool next>
void cycle_buffer(const ParametersParser& parser,
                  Context& context,
                  const ShellContext&) {
...
template <bool force>
void delete_buffer(const ParametersParser& parser,
                   Context& context,
                   const ShellContext&) {
...
Completions complete_highlighter(const Context& context,
  ...
  if (shared) {
    ...
    return offset_pos(
        group.complete_child(arg.substr(1_byte), pos_in_token - 1, only_group),
        1);
  } else {
...
Completions rm_highlighter_completer(const Context& context,
  ...
  if (token_to_complete == 0 and not arg.empty() and arg.front() == '/') {
    auto& group = DefinedHighlighters::instance();
    return offset_pos(
        group.complete_child(arg.substr(1_byte), pos_in_token - 1, false), 1);
...
void define_command(const ParametersParser& parser,
                    Context& context,
                    const ShellContext&) {
  ...
  if (auto params = parser.get_switch("params")) {
    ...
    desc =
        ParameterDesc{{}, ParameterDesc::Flags::SwitchesAsPositional, min, max};
    ...
    completer = [=](const Context& context, CompletionFlags flags,
      ...
      String output =
          ShellManager::instance()
              .eval(shell_cmd, context, {}, ShellManager::Flags::WaitForStdout,
                    shell_context)
              .first;
...
const CommandDesc echo_cmd = {
    ...
    [](const ParametersParser& parser, Context& context, const ShellContext&) {
      ...
      if (parser.get_switch("debug"))
        ...
        auto face =
            get_face(parser.get_switch("color").value_or("StatusLine").str());
...
const CommandDesc debug_cmd = {
    ...
    CommandFlags::None, CommandHelper{},
    PerArgumentCommandCompleter({[](const Context& context,
                                    CompletionFlags flags,
                                    const String& prefix,
    ...
    [](const ParametersParser& parser, Context& context, const ShellContext&) {
      if (parser[0] == "info") {
        ...
        for (int domain = 0; domain < (int)MemoryDomain::Count; ++domain) {
          ...
          total += count;
          write_to_debug_buffer(
              format("  {}: {}", domain_name((MemoryDomain)domain), count));
        }
...
static OptionManager& get_options(StringView scope,
                                  const Context& context,
...
const CommandDesc set_option_cmd = {
    "set", nullptr,
    "set <switches> <scope> <name> <value>: set option <name> in <scope> to "
    "<value>\n"
...
const CommandDesc map_key_cmd = {
    "map", nullptr,
    "map <scope> <mode> <key> <keys>: map <key> to <keys> in given mode in "
    "<scope>.\n"
    "<mode> can be:\n"
    "    normal\n"
    "    insert\n"
    ...
    [](const Context& context,
      ...
      if (token_to_complete == 1) {
        constexpr const char* modes[] = {"normal", "insert", "menu", "prompt",
...
void context_wrap(const ParametersParser& parser, Context& context, Func func) {
  ...
  if (parser.get_switch("draft")) {
    InputHandler input_handler(real_context->selections(),
                               Context::Flags::Transient, real_context->name());
    Context& c = input_handler.context();

    // We do not want this draft context to commit undo groups if the real one
    // is
...
const CommandDesc exec_string_cmd = {
    "exec", nullptr,
    "exec <switches> <keys>: execute given keys in normal mode as if entered "
    "by user",
...
const CommandDesc prompt_cmd = {
    "prompt", nullptr,
    "prompt <prompt> <register> <command>: prompt the use to enter a text "
    "string "
    ...
    [](const ParametersParser& parser,
      ...
      if (parser.get_switch("file-completion"))
        ...
        completer = [](const Context& context, CompletionFlags,
                       StringView prefix, ByteCount cursor_pos) -> Completions {
          return {0_byte, cursor_pos,
                  ClientManager::instance().complete_client_name(prefix,
                                                                 cursor_pos)};
        };
      else if (parser.get_switch("buffer-completion"))
        completer = [](const Context& context, CompletionFlags,
                       StringView prefix, ByteCount cursor_pos) -> Completions {
          return {0_byte, cursor_pos, complete_buffer_name(prefix, cursor_pos)};
        };
      else if (parser.get_switch("command-completion"))
        completer = [](const Context& context, CompletionFlags flags,
                       StringView prefix, ByteCount cursor_pos) -> Completions {
          return CommandManager::instance().complete(context, flags, prefix,
                                                     cursor_pos);
        };
...
const CommandDesc onkey_cmd = {
    "onkey", nullptr,
    "onkey <register> <command>: wait for next user key, store it in "
    "<register> and execute <command>",
    ParameterDesc{{}, ParameterDesc::Flags::None, 2, 2}, CommandFlags::None,
    CommandHelper{}, CommandCompleter{}, [](const ParametersParser& parser,
                                            Context& context,
                                            const ShellContext& shell_context) {
      String reg = parser[0];
...
const CommandDesc info_cmd = {
    "info", nullptr,
    "info <switches> <params>...: display an info box with the params as "
    "content",
...
static Completions complete_face(const Context&,
                                 CompletionFlags flags,
                                 const String& prefix,
                                 ByteCount cursor_pos) {
  return {0_byte, cursor_pos,
