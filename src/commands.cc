...
Buffer *open_fifo(StringView name, StringView filename, bool scroll)
{
    ...
    if (fd < 0) throw runtime_error(format("unable to open '{}'", filename));

    return create_fifo_buffer(name.str(), fd, scroll);
}

const PerArgumentCommandCompleter filename_completer(
    {[](const Context &context, CompletionFlags flags, const String &prefix,
...
static CandidateList complete_buffer_name(StringView query,
                                          ByteCount cursor_pos)
{
    ...
    for (auto &match : matches) res.push_back(match.buffer->display_name());
...
{
    if (auto s = get_scope_ifp(scope, context)) return *s;
          ...
          const ShellContext &)
{
    ...
    else
    {
        ...
        {
            if (buffer and
                (force_reload or buffer->flags() != Buffer::Flags::None))
            {
            ...
            if (not buffer)
                buffer =
                    buffer_manager.create_buffer(name, Buffer::Flags::None);
        }
        ...
        else if (not buffer)
        {
            buffer = parser.get_switch("existing")
                         ? open_file_buffer(name)
                         : open_or_create_file_buffer(name);
            if (buffer->flags() & Buffer::Flags::New)
                context.print_status(
                    {format("new file '{}'", name), get_face("StatusLine")});
        }
    }

    const size_t param_count = parser.positional_count();
    if (buffer != &context.buffer() or param_count > 1) context.push_jump();

    if (buffer != &context.buffer()) context.change_buffer(*buffer);
...
ParameterDesc edit_params{
    {{"existing",
     ...
     {"scroll",
      {false,
       "place the initial cursor so that the fifo will scroll "
       "to show new data"}}},
    ParameterDesc::Flags::None,
    0,
    3};
const CommandDesc edit_cmd = {"edit", "e",
                              "edit [<switches>] <filename> "
                              "[<line> [<column>]]: open the "
                              "given filename in a buffer",
                              edit_params, CommandFlags::None, CommandHelper{},
                              filename_completer, edit<false>};

const CommandDesc force_edit_cmd = {
    "edit!", "e!",
    "edit! [<switches>] <filename> [<line> [<column>]]: open "
    "the given filename in a buffer, "
    "force reload if needed",
    edit_params, CommandFlags::None, CommandHelper{}, filename_completer,
    edit<true>};
                  ...
                  const ShellContext &)
{
    ...
    auto filename = parser.positional_count() == 0 ? buffer.name()
...
const CommandDesc write_cmd = {
    ...
    single_optional_name_param, CommandFlags::None, CommandHelper{},
    filename_completer, write_buffer,
};
...
const CommandDesc writeall_cmd = {
    "writeall", "wa", "write all buffers that are associated to a file",
    no_params, CommandFlags::None, CommandHelper{}, CommandCompleter{},
...
const CommandDesc kill_cmd = {
    "kill", nullptr, "kill current session, quit all clients and server",
    no_params, CommandFlags::None, CommandHelper{}, CommandCompleter{},
    [](const ParametersParser &, Context &, const ShellContext &) {
...
const CommandDesc force_kill_cmd = {
    "kill!", nullptr,
    "kill current session, quit all clients and server, do "
    "not check for unsaved buffers",
    no_params, CommandFlags::None, CommandHelper{}, CommandCompleter{},
    [](const ParametersParser &, Context &, const ShellContext &) {
        throw kill_session{};
    }};

template <bool force> void quit()
{
...
const CommandDesc writeall_quit_cmd = {
    "waq", nullptr,
    "write all buffers associated to a file and quit current client", no_params,
    CommandFlags::None, CommandHelper{}, CommandCompleter{},
...
const CommandDesc buffer_cmd = {
    "buffer", "b", "buffer <name>: set buffer to edit in current client",
    single_name_param, CommandFlags::None, CommandHelper{}, buffer_completer,
    [](const ParametersParser &parser, Context &context, const ShellContext &) {
                   ...
                   const ShellContext &)
{
    ...
    if (manager.count() == 1)
        throw runtime_error(
            format("buffer '{}' is the last one", buffer.name()));
...
const CommandDesc force_delbuf_cmd = {
    ...
    single_optional_name_param, CommandFlags::None, CommandHelper{},
    buffer_completer, delete_buffer<true>};

const CommandDesc namebuf_cmd = {
    "namebuf", nullptr, "namebuf <name>: change current buffer name",
    single_name_param, CommandFlags::None, CommandHelper{}, CommandCompleter{},
    [](const ParametersParser &parser, Context &context, const ShellContext &) {
...
Completions complete_highlighter(const Context &context, StringView arg,
                                 ByteCount pos_in_token, bool only_group)
{
    const bool shared = not arg.empty() and arg[0_byte] == '/';
    if (shared)
    {
        auto &group = DefinedHighlighters::instance();
        return offset_pos(group.complete_child(arg.substr(1_byte),
                                               pos_in_token - 1, only_group),
                          1);
    }
    else
    {
...
Completions rm_highlighter_completer(const Context &context,
                                     ...
                                     ByteCount pos_in_token)
{
    ...
    if (token_to_complete == 0 and not arg.empty() and arg.front() == '/')
    {
        auto &group = DefinedHighlighters::instance();
        return offset_pos(
            group.complete_child(arg.substr(1_byte), pos_in_token - 1, false),
            1);
    }
    else if (token_to_complete == 0)
        return context.window().highlighters().complete_child(arg, pos_in_token,
                                                              false);
    return {};
}

Completions add_highlighter_completer(const Context &context,
                                      CompletionFlags flags,
                                      CommandParameters params,
                                      size_t token_to_complete,
                                      ByteCount pos_in_token)
{
    ...
    else if (token_to_complete == 0 or
             (token_to_complete == 2 and params[0] == "-group"))
        return {0_byte, arg.length(),
                complete(arg, pos_in_token,
                         HighlighterRegistry::instance() |
                             transform(HighlighterRegistry::get_id))};
    return Completions{};
}

Highlighter &get_highlighter(const Context &context, StringView path)
{
    if (path.empty()) throw runtime_error("group path should not be empty");
    ...
    if (path.back() == '/') path = path.substr(0_byte, path.length() - 1);

    if (not path.empty()) return root->get_child(path);
    return *root;
}

const CommandDesc add_highlighter_cmd = {
    "addhl", "ah", "addhl <type> <type params>...: add an highlighter",
    ParameterDesc{{{"group",
                    {true,
                     "Set the group in which to put the highlighter. "
    ...
    [](const ParametersParser &parser, Context &context, const ShellContext &) {
        ...
        if (it == registry.end())
            throw runtime_error(
                format("No such highlighter factory '{}'", name));
        group.add_child(it->value.factory(highlighter_params));

        if (context.has_window()) context.window().force_redraw();
    }};

const CommandDesc rm_highlighter_cmd = {
    "rmhl", "rh", "rmhl <path>: remove highlighter <name>",
    ParameterDesc{{}, ParameterDesc::Flags::None, 1, 1}, CommandFlags::None,
    CommandHelper{}, rm_highlighter_completer,
    [](const ParametersParser &parser, Context &context, const ShellContext &) {
...
const CommandDesc add_hook_cmd = {
    ...
    ParameterDesc{{{"group", {true, "set hook group, see rmhooks"}}},
                  ParameterDesc::Flags::None,
                  4,
                  4},
                    ...
                    const ShellContext &)
{
    ...
    if (cm.command_defined(cmd_name) and
        not parser.get_switch("allow-override"))
        throw runtime_error(format("command '{}' already defined", cmd_name));

    CommandFlags flags = CommandFlags::None;
    if (parser.get_switch("hidden")) flags = CommandFlags::Hidden;
    ...
    else if (auto shell_cmd_opt = parser.get_switch("shell-candidates"))
    {
        ...
        completer = [shell_cmd, candidates, token](
            ...
            {
                if (flags ==
                    CompletionFlags::Fast)  // no shell on fast completion
                    return Completions{};
            ...
            if (token != token_to_complete) return Completions{};
            ...
            matches.erase(std::unique(matches.begin(), matches.end()),
                          matches.end());
            CandidateList res;
            for (auto &m : matches) res.push_back(m.candidate().str());
...
const CommandDesc define_command_cmd = {
    ...
    ParameterDesc{
        {{"params",
         ...
         {"shell-completion",
          {true, "complete the parameters using the given shell-script"}},
         {"shell-candidates",
          {true, "get the parameter candidates using the given shell-script"}}},
        ParameterDesc::Flags::None,
        2,
        2},
    CommandFlags::None, CommandHelper{}, CommandCompleter{}, define_command};
...
const CommandDesc alias_cmd = {
    ...
    ParameterDesc{{}, ParameterDesc::Flags::None, 3, 3}, CommandFlags::None,
    CommandHelper{},
    ...
    [](const ParametersParser &parser, Context &context, const ShellContext &) {
        if (not CommandManager::instance().command_defined(parser[2]))
            throw runtime_error(
                format("Command '{}' does not exist", parser[2]));
...
const CommandDesc unalias_cmd = {
    ...
    "<expected>",
    ParameterDesc{{}, ParameterDesc::Flags::None, 2, 3}, CommandFlags::None,
    CommandHelper{},
    PerArgumentCommandCompleter(
        {complete_scope, complete_nothing, complete_command_name
...
const CommandDesc echo_cmd = {
    ...
    [](const ParametersParser &parser, Context &context, const ShellContext &) {
        ...
        {
            auto face = get_face(
                parser.get_switch("color").value_or("StatusLine").str());
...
const CommandDesc debug_cmd = {
    ...
    [](const ParametersParser &parser, Context &context, const ShellContext &) {
        ...
        else if (parser[0] == "shared-strings")
        {
            ...
            throw runtime_error(
                format("unknown debug command '{}'", parser[0]));
    }};

const CommandDesc source_cmd = {
    "source", nullptr,
    "source <filename>: execute commands contained in <filename>",
    single_name_param, CommandFlags::None, CommandHelper{}, filename_completer,
    [](const ParametersParser &parser, Context &context, const ShellContext &) {
                                ...
                                CommandParameters params)
{
    ...
    auto desc =
        GlobalScope::instance().option_registry().option_desc(params[1]);
    if (not desc or desc->docstring().empty()) return "";
...
{
    if (scope == "current") return context.options()[option_name].manager();
    return get_scope(scope, context).options();
}

const CommandDesc set_option_cmd = {
    "set", nullptr,
    "set <switches> <scope> <name> <value>: set option <name> "
    "in <scope> to <value>\n"
    "<scope> can be global, buffer, window, or current which "
    ...
    ParameterDesc{{{"add", {false, "add to option rather than replacing it"}}},
                  ...
                  3,
                  3},
    CommandFlags::None, option_doc_helper,
...
const CommandDesc unset_option_cmd = {
    "unset", nullptr,
    "unset <scope> <name>: remove <name> option from scope, falling back on "
    "parent scope value"
...
const CommandDesc declare_option_cmd = {
    ...
    [](const ParametersParser &parser, Context &context, const ShellContext &) {
        ...
        if (parser.get_switch("hidden")) flags = OptionFlags::Hidden;

        auto docstring =
            parser.get_switch("docstring").value_or(StringView{}).str();
        ...
        if (parser[0] == "int")
            ...
            opt = &reg.declare_option<TimestampedList<LineAndFlag>>(
                parser[1], docstring, {}, flags);
        else if (parser[0] == "range-faces")
            opt = &reg.declare_option<TimestampedList<RangeAndFace>>(
                parser[1], docstring, {}, flags);
        else
            throw runtime_error(format("unknown type {}", parser[0]));

        if (parser.positional_count() == 3) opt->set_from_string(parser[2]);
    }};
...
const CommandDesc map_key_cmd = {
    "map", nullptr,
    "map <scope> <mode> <key> <keys>: map <key> to <keys> in "
    "given mode in <scope>.\n"
    "<mode> can be:\n"
    "    normal\n"
    "    insert\n"
    ...
    "    user\n"
    "    object\n",
    ParameterDesc{{}, ParameterDesc::Flags::None, 4, 4}, CommandFlags::None,
    CommandHelper{},
    [](const Context &context, CompletionFlags flags, CommandParameters params,
       size_t token_to_complete, ByteCount pos_in_token) -> Completions {
        if (token_to_complete == 0)
        ...
        {
            constexpr const char *modes[] = {"normal", "insert", "menu",
                                             "prompt", "goto",   "view",
                                             "user",   "object"};
...
const ParameterDesc context_wrap_params = {
    {{"client", {true, "run in given client context"}},
     {"try-client",
      {true,
       "run in given client context if it exists, or else in the current one"}},
     {"buffer",
      {true,
       "run in a disposable context for each given buffer in "
       "the comma separated list argument"}},
     ...
     {"collapse-jumps",
      {false, "collapse all jumps into a single one from initial selection"}}},
    ParameterDesc::Flags::SwitchesOnlyAtStart,
    1};

template <typename T> struct DisableOption
{
    DisableOption(Context &context, const char *name)
        : m_option(context.options()[name]), m_prev_value(m_option.get<T>())
    {
...
{
   public:
    RegisterRestorer(char name, const Context &context) : m_name(name)
    {
        ConstArrayView<String> save =
            RegisterManager::instance()[name].values(context);
    ...
    ~RegisterRestorer()
    {
        if (m_name != 0) RegisterManager::instance()[m_name] = m_save;
    }
...
void context_wrap(const ParametersParser &parser, Context &context, Func func)
{
    ...
    const bool no_hooks =
        parser.get_switch("no-hooks") or context.user_hooks_disabled();
    ...
    if (auto bufnames = parser.get_switch("buffer"))
    {
        ...
        if (*bufnames == "*")
        {
            ...
            for (auto buffer : buffers) context_wrap_for_buffer(*buffer);
        }
        else
    ...
    if (draft)
    {
        input_handler.emplace(base_context->selections(),
                              Context::Flags::Transient, base_context->name());
    ...
    if (parser.get_switch("itersel"))
    {
        ...
        for (auto &sel : sels)
        {
            ...
            if (&sels.buffer() != &c.buffer())
                throw runtime_error(
                    "the buffer has changed while iterating on selections");
        }
    ...
    else
    {
        const bool collapse_jumps =
            not draft and (bool) parser.get_switch("collapse-jumps");
        SelectionList jump = c.selections();
        JumpList original_jump_list =
            collapse_jumps ? c.jump_list() : JumpList{};
...
const CommandDesc exec_string_cmd = {
    "exec", nullptr,
    "exec <switches> <keys>: execute given keys in normal "
    "mode as if entered by user",
    context_wrap_params, CommandFlags::None, CommandHelper{},
    CommandCompleter{},
    [](const ParametersParser &parser, Context &context, const ShellContext &) {
        context_wrap(parser, context, [](const ParametersParser &parser,
                                         Context &context) {
            KeyList keys;
            ...
            for (auto &key : keys) context.input_handler().handle_key(key);
        });
    }};
...
const CommandDesc prompt_cmd = {
    "prompt", nullptr,
    "prompt <prompt> <register> <command>: prompt the use "
    ...
    [](const ParametersParser &parser, Context &context,
       const ShellContext &shell_context) {
        ...
        context.input_handler().prompt(
            parser[0], initstr.str(), get_face("Prompt"), password,
            std::move(completer),
            [=](StringView str, PromptEvent event, Context &context) {
                if (event != PromptEvent::Validate) return;
                RegisterManager::instance()[reg] =
                    ConstArrayView<String>(str.str());
                ...
                {
                    const String &str =
                        RegisterManager::instance()[reg].values(context)[0];
                    memset(const_cast<String &>(str).data(), 0,
                           (int)str.length());
                    RegisterManager::instance()[reg] =
...
const CommandDesc menu_cmd = {
    ...
    [](const ParametersParser &parser, Context &context,
       const ShellContext &shell_context) {
        ...
        if (count == 0 or (count % modulo) != 0) throw wrong_argument_count();
        ...
        for (int i = 0; i < count; i += modulo)
        {
            ...
            if (with_select_cmds) select_cmds.push_back(parser[i + 2]);
        }
        ...
        context.input_handler().menu(
            ...
            [=](int choice, MenuEvent event, Context &context) {
                ...
                if (event == MenuEvent::Validate and choice >= 0 and
                    choice < commands.size())
                    CommandManager::instance().execute(commands[choice],
                                                       context, shell_context);
                if (event == MenuEvent::Select and choice >= 0 and
                    choice < select_cmds.size())
...
const CommandDesc onkey_cmd = {
    "onkey", nullptr,
    "onkey <register> <command>: wait for next user key, "
    "store it in <register> and execute <command>",
    ParameterDesc{{}, ParameterDesc::Flags::None, 2, 2}, CommandFlags::None,
    CommandHelper{}, CommandCompleter{},
    [](const ParametersParser &parser, Context &context,
       const ShellContext &shell_context) {
        String reg = parser[0];
        String command = parser[1];
        context.input_handler().on_next_key(
            KeymapMode::None, [=](Key key, Context &context) {
...
const CommandDesc info_cmd = {
    "info", nullptr,
    "info <switches> <params>...: display an info box with "
    ...
    CommandFlags::None, CommandHelper{}, CommandCompleter{},
    [](const ParametersParser &parser, Context &context, const ShellContext &) {
        if (not context.has_client()) return;
...
const CommandDesc try_catch_cmd = {
    ...
    ParameterDesc{{}, ParameterDesc::Flags::None, 1, 3}, CommandFlags::None,
    CommandHelper{}, CommandCompleter{},
    [](const ParametersParser &parser, Context &context,
       const ShellContext &shell_context) {
        if (parser.positional_count() == 2) throw wrong_argument_count();
...
const CommandDesc face_cmd = {
    ...
    [](const ParametersParser &parser, Context &context, const ShellContext &) {
        ...
        for (auto &client : ClientManager::instance()) client->force_redraw();
    }};

const CommandDesc set_client_name_cmd = {
    "nameclient", "nc", "nameclient <name>: set current client name to <name>",
    single_name_param, CommandFlags::None, CommandHelper{}, CommandCompleter{},
    [](const ParametersParser &parser, Context &context, const ShellContext &) {
        if (ClientManager::instance().validate_client_name(parser[0]))
            ...
            throw runtime_error(
                format("client name '{}' is not unique", parser[0]));
    }};

const CommandDesc set_register_cmd = {
    "reg", nullptr, "reg <name> <value>: set register <name> to <value>",
    ParameterDesc{{}, ParameterDesc::Flags::None, 2, 2}, CommandFlags::None,
    CommandHelper{}, CommandCompleter{},
    [](const ParametersParser &parser, Context &context, const ShellContext &) {
        RegisterManager::instance()[parser[0]] =
            ConstArrayView<String>(parser[1]);
    }};

const CommandDesc select_cmd = {
    "select", nullptr, "select <selections_desc>: select given selections",
    ParameterDesc{{}, ParameterDesc::Flags::None, 1, 1}, CommandFlags::None,
    CommandHelper{}, CommandCompleter{},
    [](const ParametersParser &parser, Context &context, const ShellContext &) {
        context.selections_write_only() =
            selection_list_from_string(context.buffer(), parser[0]);
    }};

const CommandDesc change_working_directory_cmd = {
    "cd", nullptr, "cd <dir>: change server working directory to <dir>",
    single_optional_name_param, CommandFlags::None, CommandHelper{},
    PerArgumentCommandCompleter{
        {[](const Context &context, CompletionFlags flags, const String &prefix,
            ByteCount cursor_pos) -> Completions {
    ...
    [](const ParametersParser &parser, Context &, const ShellContext &) {
        ...
        if (chdir(parse_filename(target).c_str()) != 0)
            throw runtime_error(
                format("cannot change to directory '{}'", target));
        for (auto &buffer : BufferManager::instance())
