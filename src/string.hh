...
class StringOps {
 public:
  ...
  [[gnu::always_inline]] const_iterator begin() const { return type().data(); }
...
class StringView : public StringOps<StringView, const char> {
 public:
  ...
  [[gnu::always_inline]] constexpr ByteCount length() const { return m_length; }
...
inline StringView StringOps<Type, CharType>::substr(ByteCount from,
  ...
  return StringView{type().data() + (int)from,
                    std::min(type().length() - from, length)};
}
...
decltype(to_string(std::declval<ValueType>())) to_string(
    const StronglyTypedNumber<RealType, ValueType>& val) {
...
template <typename T>
using IsString = std::is_convertible<T, StringView>;

template <typename T,
          class = typename std::enable_if<!IsString<T>::value>::type>
auto format_param(const T& val) -> decltype(to_string(val)) {
...
String format(StringView fmt, Types... params) {
  return format(fmt,
                ArrayView<const StringView>{detail::format_param(params)...});
}

StringView format_to(ArrayView<char> buffer,
                     StringView fmt,
                     ArrayView<const StringView> params);

template <typename... Types>
StringView format_to(ArrayView<char> buffer, StringView fmt, Types... params) {
  return format_to(buffer, fmt, ArrayView<const StringView>{
                                    detail::format_param(params)...});
}
}
