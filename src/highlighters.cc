...
{
template <typename T>
void highlight_range(DisplayBuffer &display_buffer, ByteCoord begin,
                     ByteCoord end, bool skip_replaced, T func)
{
    ...
    for (auto &line : display_buffer.lines())
    {
        ...
        if (range.end <= begin or end < range.begin) continue;

        for (auto atom_it = line.begin(); atom_it != line.end(); ++atom_it)
        {
            bool is_replaced =
                atom_it->type() == DisplayAtom::ReplacedBufferRange;

            if (not atom_it->has_buffer_range() or
                (skip_replaced and is_replaced))
                continue;

            if (end <= atom_it->begin() or begin >= atom_it->end()) continue;
...
void apply_highlighter(const Context &context, HighlightFlags flags,
                       DisplayBuffer &display_buffer, ByteCoord begin,
                       ByteCoord end, Highlighter &highlighter)
{
    if (begin == end) return;
         ...
         ++line_it)
    {
        ...
        if (region_lines.empty()) first_line = line_it;
    ...
    if (region_display.lines().empty()) return;
...
{
    if (params.size() != 1) throw runtime_error("wrong parameter count");
...
template <typename T> struct BufferSideCache
{
    ...
    T &get(const Buffer &buffer)
    {
        ...
        if (not cache_val) cache_val = Value(T{});
...
class RegexHighlighter : public Highlighter
{
   public:
    ...
    void highlight(const Context &context, HighlightFlags flags,
                   DisplayBuffer &display_buffer, BufferRange range) override
    {
        ...
        if (flags != HighlightFlags::Highlight or
        ...
        auto &matches =
            get_matches(context.buffer(), display_buffer.range(), range);
    ...
    {
        if (params.size() < 2) throw runtime_error("wrong parameter count");
        ...
        return {id,
                make_unique<RegexHighlighter>(std::move(ex), std::move(faces))};
    }
   ...
   private:
                     ...
                     BufferRange range)
    {
        ...
        RegexIt re_it{
            buffer.iterator_at(range.begin), buffer.iterator_at(range.end),
            m_regex, match_flags(is_bol(range.begin), is_eol(buffer, range.end),
                           ...
                           BufferRange buffer_range)
    {
        ...
        BufferRange range{
            std::max<ByteCoord>(buffer_range.begin,
                                display_range.begin.line - line_offset),
            std::min<ByteCoord>(buffer_range.end,
                                display_range.end.line + line_offset)};

        auto it = std::upper_bound(
            matches.begin(), matches.end(), range,
...
class DynamicRegexHighlighter : public Highlighter
{
   public:
    ...
    void highlight(const Context &context, HighlightFlags flags,
                   DisplayBuffer &display_buffer, BufferRange range)
    {
        if (flags != HighlightFlags::Highlight) return;
...
HighlighterAndId create_dynamic_regex_highlighter(HighlighterParameters params)
{
    ...
    auto get_face = [faces](const Context &context) {
        return faces;
        ;
    };
            ...
            .is_of_type<Regex>())
    {
        ...
        return {format("dynregex_{}", expr),
                make_dynamic_regex_highlighter(get_regex, get_face)};
    }
    ...
    auto get_regex = [expr](const Context &context) {
        ...
        catch (runtime_error &err)
        {
            write_to_debug_buffer(format(
                "Error while evaluating dynamic regex expression", err.what()));
            return Regex{};
        }
    };
    return {format("dynregex_{}", expr),
...
HighlighterAndId create_line_highlighter(HighlighterParameters params)
{
    ...
    auto func = [=](const Context &context, HighlightFlags flags,
        ...
        if (it == display_buffer.lines().end()) return;
        ...
        for (auto atom_it = it->begin(); atom_it != it->end(); ++atom_it)
        {
            ...
            if (!atom_it->has_buffer_range()) continue;

            kak_assert(atom_it->begin().line == line);
            apply_face(face)(*atom_it);
        }
        const CharCount remaining =
            context.window().dimensions().column - column;
        if (remaining > 0) it->push_back({String{' ', remaining}, face});
    };
...
HighlighterAndId create_column_highlighter(HighlighterParameters params)
{
    ...
    auto func = [=](const Context &context, HighlightFlags flags,
        ...
        for (auto &line : display_buffer.lines())
        {
            ...
            const ByteCount byte_col =
                get_byte_to_column(buffer, tabstop, {buf_line, column});
            ...
            if (not found)
            {
                ...
                for (auto &atom : line)
                {
                    ...
                    {
                        first_buffer_col =
                            get_column(buffer, tabstop, atom.begin());
                        break;
                    }
                    first_display_col += atom.length();
                }

                if (first_buffer_col == -1) continue;

                CharCount eol_col = line.length();
                CharCount count =
                    column + first_display_col - first_buffer_col - eol_col;
                if (count >= 0)
                {
                    if (count > 0) line.push_back({String{' ', count}});
                    line.push_back({String{" "}, face});
                }
                else
                {
                    for (auto atom_it = line.end();
                         atom_it != line.begin() and count < 0; --atom_it)
                    {
                        ...
                        DisplayBuffer &display_buffer, BufferRange)
{
    ...
    for (auto &line : display_buffer.lines())
    {
        for (auto atom_it = line.begin(); atom_it != line.end(); ++atom_it)
        {
            if (atom_it->type() != DisplayAtom::BufferRange) continue;
...
void show_whitespaces(const Context &context, HighlightFlags flags,
                      DisplayBuffer &display_buffer, BufferRange)
{
    const int tabstop = context.options()["tabstop"].get<int>();
        ...
        for (auto atom_it = line.begin(); atom_it != line.end(); ++atom_it)
        {
            ...
            for (BufferIterator it = begin; it != end; ++it)
            {
                ...
                if (c == '\t' or c == ' ' or c == '\n')
                {
                    ...
                    {
                        int column =
                            (int)get_column(buffer, tabstop, it.coord());
                        int count = tabstop - (column % tabstop);
...
HighlighterAndId create_show_whitespaces_highlighter(
    HighlighterParameters params)
{
...
void show_line_numbers(const Context &context, HighlightFlags flags,
                       DisplayBuffer &display_buffer, BufferRange,
                       bool relative, bool hl_cursor_line, StringView separator)
{
    ...
    int digit_count = 0;
    for (LineCount c = last_line; c > 0; c /= 10) ++digit_count;
...
void show_matching_char(const Context &context, HighlightFlags flags,
                        DisplayBuffer &display_buffer, BufferRange)
{
    ...
    {
        auto pos = sel.cursor();
        if (pos < range.begin or pos >= range.end) continue;
        ...
        for (auto &pair : matching_chars)
        {
            ...
            if (c == pair.first)
            {
                for (auto it = buffer.iterator_at(pos) + 1,
                          end = buffer.iterator_at(range.end);
                     it != end; ++it)
                {
                    char c = *it;
                    if (c == pair.first)
                    ...
                    {
                        highlight_range(display_buffer, it.coord(),
                                        (it + 1).coord(), false,
                                        apply_face(face));
                        break;
                    }
            ...
            {
                for (auto it = buffer.iterator_at(pos) - 1,
                          end = buffer.iterator_at(range.begin);
                     true; --it)
                {
                    char c = *it;
                    if (c == pair.second)
                          ...
                          DisplayBuffer &display_buffer, BufferRange)
{
    ...
    for (size_t i = 0; i < context.selections().size(); ++i)
    {
        ...
        ByteCoord end =
            forward ? (ByteCoord)sel.cursor() : buffer.char_next(sel.anchor());
    ...
    for (size_t i = 0; i < context.selections().size(); ++i)
    {
        ...
        const bool primary = (i == context.selections().main_index());
        highlight_range(
            display_buffer, sel.cursor(), buffer.char_next(sel.cursor()), false,
        ...
        for (auto atom_it = line.begin(); atom_it != line.end(); ++atom_it)
        {
            if (atom_it->type() == DisplayAtom::BufferRange)
            {
                for (auto it = buffer.iterator_at(atom_it->begin()),
                          end = buffer.iterator_at(atom_it->end());
                     it < end;)
                {
                    auto coord = it.coord();
                    Codepoint cp =
                        utf8::read_codepoint<utf8::InvalidPolicy::Pass>(it,
                                                                        end);
...
HighlighterAndId create_flag_lines_highlighter(HighlighterParameters params)
{
    ...
    GlobalScope::instance()
        .options()[option_name]
        .get<TimestampedList<LineAndFlag>>();

    auto func = [=](const Context &context, HighlightFlags flags,
                    DisplayBuffer &display_buffer, BufferRange) {
        auto &line_flags = context.options()[option_name]
                               .get_mutable<TimestampedList<LineAndFlag>>();
        ...
        if (line_flags.prefix != buffer.timestamp())
        {
            ...
            for (auto it = lines.begin(); it != lines.end(); ++it)
            {
                ...
                if (ins_pos != it) *ins_pos = std::move(*it);
                ++ins_pos;
            }
        ...
        CharCount width = 0;
        for (auto &l : display_lines) width = std::max(width, l.length());
        const DisplayAtom empty{String{' ', width}, def_face};
        for (auto &line : display_buffer.lines())
        {
            int line_num = (int)line.range().begin.line + 1;
            auto it = find_if(lines, [&](const LineAndFlag &l) {
            ...
            else
            {
                ...
                if (padding_atom.length() != 0) *it++ = std::move(padding_atom);
            }
        }
    };
...
HighlighterAndId create_ranges_highlighter(HighlighterParameters params)
{
    ...
    GlobalScope::instance()
        .options()[option_name]
        .get<TimestampedList<RangeAndFace>>();

    auto func = [=](const Context &context, HighlightFlags flags,
                    DisplayBuffer &display_buffer, BufferRange) {
        auto &range_and_faces =
            context.options()[option_name]
                .get_mutable<TimestampedList<RangeAndFace>>();
...
HighlighterAndId create_reference_highlighter(HighlighterParameters params)
{
    ...
    auto func = [=](const Context &context, HighlightFlags flags,
        ...
        try
        {
            DefinedHighlighters::instance().get_child(name).highlight(
                context, flags, display_buffer, range);
        }
...
void find_matches(const Buffer &buffer, RegexMatchList &matches,
                  const Regex &regex)
{
                    ...
                    RegexMatchList &matches, const Regex &regex)
{
    ...
    {
        auto modif_it =
            std::upper_bound(modifs.begin(), modifs.end(), it->line,
                             [](const LineCount &l, const LineModification &c) {
                                 return l < c.old_line;
                             });
    ...
    for (auto &modif : modifs)
    {
        for (auto line = modif.new_line;
             line < modif.new_line + modif.num_added; ++line)
        {
            auto l = buffer[line];
            for (RegexIterator<const char *> it{l.begin(), l.end(), regex},
                 end{};
                 it != end; ++it)
            {
                ByteCount b = (int)((*it)[0].first - l.begin());
...
struct RegionMatches
{
    ...
    RegexMatchList::const_iterator find_matching_end(ByteCoord beg_pos) const
    {
        ...
        while (true)
        {
            ...
            if (end_it == end_matches.end()) return end_it;
            ...
            if (recurse_level == 0) return end_it;
...
struct RegionDesc
{
                        ...
                        RegionMatches &matches) const
    {
        ...
        if (not m_recurse.empty())
            Kakoune::update_matches(buffer, modifs, matches.recurse_matches,
                                    m_recurse);
    }
};
...
{
   public:
    using NamedRegionDescList =
        Vector<std::pair<String, RegionDesc>, MemoryDomain::Highlight>;
                   ...
                   DisplayBuffer &display_buffer, BufferRange range) override
    {
        ...
        auto last_begin =
            (begin == regions.begin()) ? ByteCoord{0, 0} : (begin - 1)->end;
        for (; begin != end; ++begin)
        {
    ...
    static HighlighterAndId create(HighlighterParameters params)
    {
        ...
        if ((parser.positional_count() % 4) != 1)
            throw runtime_error(
                "wrong parameter count, expect <id> (<group "
                "name> <begin> <end> <recurse>)+");

        RegionsHighlighter::NamedRegionDescList regions;
        for (size_t i = 1; i < parser.positional_count(); i += 4)
        {
            if (parser[i].empty() or parser[i + 1].empty() or
                parser[i + 2].empty())
                throw runtime_error(
                    "group id, begin and end must not be empty");
            ...
            regions.push_back(
                {parser[i],
                 {std::move(begin), std::move(end), std::move(recurse)}});
        }

        auto default_group =
            parser.get_switch("default").value_or(StringView{}).str();
   ...
   private:
                                            ...
                                            BufferRange range)
    {
        ...
        if (cache.timestamp != buf_timestamp)
        {
            ...
            {
                auto modifs =
                    compute_line_modifications(buffer, cache.timestamp);
                for (size_t i = 0; i < m_regions.size(); ++i)
                    m_regions[i].second.update_matches(buffer, modifs,
                                                       cache.matches[i]);
            }
        ...
        if (it != cache.regions.end()) return it->second;
             ...
             begin != end;)
        {
            ...
            if (end_it == matches.end_matches.end() or
                end_it->end_coord() >= range.end)
            {
            ...
            {
                regions.push_back({beg_it->begin_coord(), end_it->end_coord(),
                                   named_region.first});
    ...
    registry.append(
        {"flag_lines",
         {create_flag_lines_highlighter,
          ...
          "A line-flag is written: <line>|<fg color>|<text>, the list is : "
          "separated"}});
    registry.append({"ranges",
                     {create_ranges_highlighter,
                      "Parameters: <option name>\n"
                      "Use the range-faces option given as "
                      "parameter to highlight buffer\n"}});
    registry.append({"line",
                     {create_line_highlighter,
                      "Parameters: <value string> <face>\n"
                      "Highlight the line given by evaluating "
                      "<value string> with <face>"}});
    registry.append({"column",
                     {create_column_highlighter,
                      "Parameters: <value string> <face>\n"
                      "Highlight the column given by evaluating "
                      "<value string> with <face>"}});
    registry.append(
        {"ref",
    ...
    registry.append(
        {"regions",
         {RegionsHighlighter::create,
          "Parameters: [-default <default group>] <name> {<region name> "
          "<begin> <end> <recurse>}..."
          "Split the highlighting into regions defined by the <begin>, <end> "
          "and <recurse> regex\n"
          "The region <region name> starts at <begin> match, end at <end> "
          "match that does not\n"
          "close a <recurse> match. In between region is the <default group>.\n"
          "Highlighting a region is done by adding highlighters into the "
          "different <region name> subgroups."}});
}
}
