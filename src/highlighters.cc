...
void highlight_range(DisplayBuffer& display_buffer,
  ...
  for (auto& line : display_buffer.lines()) {
    ...
    for (auto atom_it = line.begin(); atom_it != line.end(); ++atom_it) {
      ...
      if (not atom_it->has_buffer_range() or (skip_replaced and is_replaced))
        continue;
...
void apply_highlighter(const Context& context,
  ...
  for (auto line_it = display_buffer.lines().begin(); line_it != line_end;
    ...
    if (range.begin < begin or range.end > end) {
      ...
      for (auto atom_it = line.begin(); atom_it != line.end(); ++atom_it) {
        if (not atom_it->has_buffer_range() or end <= atom_it->begin() or
            begin >= atom_it->end())
          continue;
...
auto apply_face = [](const Face& face) {
  return
      [&face](DisplayAtom& atom) { atom.face = merge_faces(atom.face, face); };
};
...
class RegexHighlighter : public Highlighter {
 public:
  ...
  void highlight(const Context& context,
                 HighlightFlags flags,
                 DisplayBuffer& display_buffer,
                 BufferRange range) override {
    auto overlaps = [](const BufferRange& lhs, const BufferRange& rhs) {
      return lhs.begin < rhs.begin ? lhs.end > rhs.begin : rhs.end > lhs.begin;
    };
    ...
    for (size_t m = 0; m < matches.size(); ++m) {
      ...
      highlight_range(display_buffer, matches[m].begin, matches[m].end, true,
                      apply_face(face));
    }
  }
  ...
  static HighlighterAndId create(HighlighterParameters params) {
    ...
    return {id, std::make_unique<RegexHighlighter>(std::move(ex), std::move(faces))};
  }
 ...
 private:
  ...
  void add_matches(const Buffer& buffer,
                   MatchList& matches,
                   BufferRange range) {
    kak_assert(matches.size() % m_faces.size() == 0);
    using RegexIt = RegexIterator<BufferIterator>;
    RegexIt re_it{
        buffer.iterator_at(range.begin), buffer.iterator_at(range.end), m_regex,
        match_flags(is_bol(range.begin), is_eol(buffer, range.end),
                    is_bow(buffer, range.begin), is_eow(buffer, range.end))};
    RegexIt re_end;
  ...
  MatchList& get_matches(const Buffer& buffer,
...
make_dynamic_regex_highlighter(RegexGetter regex_getter,
                               FaceGetter face_getter) {
  return std::make_unique<DynamicRegexHighlighter<RegexGetter, FaceGetter>>(
      std::move(regex_getter), std::move(face_getter));
}

HighlighterAndId create_dynamic_regex_highlighter(
...
HighlighterAndId create_column_highlighter(HighlighterParameters params) {
  ...
  auto func = [=](const Context& context, HighlightFlags flags,
    ...
    try {
      ...
      write_to_debug_buffer(format(
          "Error evaluating highlight column expression: {}", err.what()));
    }
    ...
    for (auto& line : display_buffer.lines()) {
      ...
      if (not found) {
        ...
        if (count >= 0) {
          ...
          for (auto atom_it = line.end(); atom_it != line.begin() and count < 0;
               --atom_it) {
...
void expand_tabulations(const Context& context,
                        HighlightFlags flags,
                        DisplayBuffer& display_buffer,
                        BufferRange) {
  const int tabstop = context.options()["tabstop"].get<int>();
...
void show_whitespaces(const Context& context,
                      HighlightFlags flags,
                      DisplayBuffer& display_buffer,
                      BufferRange) {
  const int tabstop = context.options()["tabstop"].get<int>();
...
void show_line_numbers(const Context& context,
                       HighlightFlags flags,
                       DisplayBuffer& display_buffer,
                       BufferRange,
                       bool relative,
...
void show_matching_char(const Context& context,
                        HighlightFlags flags,
                        DisplayBuffer& display_buffer,
                        BufferRange) {
...
HighlighterAndId create_matching_char_highlighter(
    HighlighterParameters params) {
  return {"show_matching", make_simple_highlighter(show_matching_char)};
}

void highlight_selections(const Context& context,
                          HighlightFlags flags,
                          DisplayBuffer& display_buffer,
                          BufferRange) {
  if (flags != HighlightFlags::Highlight)
...
void expand_unprintable(const Context& context,
                        HighlightFlags flags,
                        DisplayBuffer& display_buffer,
                        BufferRange) {
  auto& buffer = context.buffer();
    ...
    for (auto atom_it = line.begin(); atom_it != line.end(); ++atom_it) {
      if (atom_it->type() == DisplayAtom::BufferRange) {
        for (auto it = buffer.iterator_at(atom_it->begin()),
          ...
          auto coord = it.coord();
          Codepoint cp =
              utf8::read_codepoint<utf8::InvalidPolicy::Pass>(it, end);
  ...
  auto func = [=](const Context& context, HighlightFlags flags,
    ...
    for (auto& line : display_buffer.lines()) {
      ...
      if (it == lines.end())
        ...
        DisplayAtom padding_atom{String(' ', width - display_line.length()),
                                 def_face};
...
HighlighterAndId create_ranges_highlighter(HighlighterParameters params) {
  ...
  auto func = [=](const Context& context, HighlightFlags flags,
                  DisplayBuffer& display_buffer, BufferRange) {
    auto& range_and_faces = context.options()[option_name]
...
void find_matches(const Buffer& buffer,
...
void update_matches(const Buffer& buffer,
  ...
  for (auto& modif : modifs) {
    for (auto line = modif.new_line; line < modif.new_line + modif.num_added;
         ++line) {
      auto l = buffer[line];
      for (RegexIterator<const char *> it{l.begin(), l.end(), regex}, end{};
           it != end; ++it) {
...
struct RegionsHighlighter : public Highlighter {
 public:
  ...
  void highlight(const Context& context,
    ...
    for (; begin != end; ++begin) {
      if (apply_default and last_begin < begin->begin)
        apply_highlighter(context, flags, display_buffer, correct(last_begin),
                          correct(begin->begin), default_group_it->value);

      auto it = m_groups.find(begin->group);
      if (it == m_groups.end())
        continue;
      apply_highlighter(context, flags, display_buffer, correct(begin->begin),
                        correct(begin->end), it->value);
      last_begin = begin->end;
    }
    if (apply_default and last_begin < display_range.end)
      apply_highlighter(context, flags, display_buffer, correct(last_begin),
                        range.end, default_group_it->value);
  }
  ...
  Completions complete_child(StringView path,
  ...
  static HighlighterAndId create(HighlighterParameters params) {
    ...
    if ((parser.positional_count() % 4) != 1)
      throw runtime_error(
          "wrong parameter count, expect <id> (<group name> <begin> <end> "
          "<recurse>)+");
    ...
    for (size_t i = 1; i < parser.positional_count(); i += 4) {
      ...
      regions.push_back(
          {parser[i], {std::move(begin), std::move(end), std::move(recurse)}});
    }

    auto default_group =
        parser.get_switch("default").value_or(StringView{}).str();
    return {parser[0],
            std::make_unique<RegionsHighlighter>(std::move(regions), default_group)};
  }
  ...
  const RegionList& get_regions_for_range(const Buffer& buffer,
    ...
    for (auto begin = find_next_begin(cache, range.begin),
      ...
      if (end_it == matches.end_matches.end() or
        ...
        regions.push_back(
            {{beg_it->line, beg_it->begin}, range.end, named_region.first});
        break;
      } else {
        regions.push_back(
            {beg_it->begin_coord(), end_it->end_coord(), named_region.first});
...
void register_highlighters() {
  ...
  registry.append({"show_matching",
                   {create_matching_char_highlighter,
                    "Apply the MatchingChar face to the char matching the one "
                    "under the cursor"}});
  ...
  registry.append({"ranges",
                   {create_ranges_highlighter,
                    ...
                    "Use the range-faces option given as parameter to "
                    "highlight buffer\n"}});
  registry.append(
      {"line",
  ...
  registry.append(
      {"regions",
       {RegionsHighlighter::create,
        "Parameters: [-default <default group>] <name> {<region name> <begin> "
        "<end> <recurse>}..."
        "Split the highlighting into regions defined by the <begin>, <end> and "
        "<recurse> regex\n"
        "The region <region name> starts at <begin> match, end at <end> match "
        "that does not\n"
