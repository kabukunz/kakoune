...
template <typename... Type> size_t hash_value(const Type &... val)
{
...
typename std::enable_if<std::is_enum<Type>::value, size_t>::type hash_value(
    const Type &val)
{
    return hash_value((typename std::underlying_type<Type>::type)val);
}

template <typename Type> size_t hash_values(Type &&t)
{
...
{
    size_t operator()(const Type &val) const { return hash_value(val); }
};
}
