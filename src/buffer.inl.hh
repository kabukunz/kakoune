...
namespace Kakoune
{
[[gnu::always_inline]] inline const char &Buffer::byte_at(ByteCoord c) const
{
...
{
    if (begin > end) return -distance(end, begin);
    if (begin.line == end.line) return end.column - begin.column;
...
{
    if (c.line < 0 or c.column < 0) return false;

    return (c.line < line_count() and c.column < m_lines[c.line].length()) or
           (c.line == line_count() - 1 and
            c.column == m_lines.back().length()) or
           (c.line == line_count() and c.column == 0);
}

inline bool Buffer::is_end(ByteCoord c) const { return c >= end_coord(); }
inline BufferIterator Buffer::begin() const { return {*this, {0_line, 0}}; }
...
inline ByteCoord Buffer::end_coord() const
{
    return m_lines.empty()
               ? ByteCoord{0, 0}
               : ByteCoord{line_count() - 1, m_lines.back().length()};
}
