...
void FDWatcher::run(EventMode mode) { m_callback(*this, mode); }
...
void EventManager::handle_next_events(EventMode mode, sigset_t *sigmask)
{
    ...
    for (int fd = 0; fd < max_fd + 1; ++fd)
    {
        if ((res > 0 and FD_ISSET(fd, &rfds)) or FD_ISSET(fd, &forced))
        {
            ...
            if (it != m_fd_watchers.end()) (*it)->run(mode);
        }
    }
    ...
    {
        if (timer->next_date() <= now) timer->run(mode);
    }
}

void EventManager::force_signal(int fd) { FD_SET(fd, &m_forced_fd); }
