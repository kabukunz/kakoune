...
template <SelectMode mode, typename T> void select(Context &context, T func)
{
    ...
    if (mode == SelectMode::Append)
    {
        ...
        if (res.captures().empty()) res.captures() = sel.captures();
...
void select_coord(Buffer &buffer, ByteCoord coord, SelectionList &selections)
{
    ...
    {
        for (auto &sel : selections) sel.cursor() = coord;
        selections.sort_and_merge_overlapping();
    }
}
        ...
        on_next_key_with_autoinfo(
            ...
            [](Key key, Context &context) {
                ...
                switch (to_lower(*cp))
                {
                    ...
                    case 'k':
                        context.push_jump();
                        select_coord<mode>(buffer, ByteCoord{0, 0},
                                           context.selections());
                        break;
                    case 'l':
                    ...
                    case 'f':
                    {
                        ...
                        if (contains_that(filename, [](char c) {
                        ...
                        if (not buffer) buffer = open_file_buffer(path);
...
template <bool lock> void view_commands(Context &context, NormalParams params)
{
    ...
    on_next_key_with_autoinfo(
        ...
        [count](Key key, Context &context) {
            ...
            if (lock) view_commands<true>(context, {});

            auto cp = key.codepoint();
            if (not cp or not context.has_window()) return;
            ...
            switch (to_lower(*cp))
            {
                ...
                case 'm':
                    context.window().center_column(
                        context.buffer()[cursor.line].char_count_to(
                            cursor.column));
                    break;
                case 't':
                    ...
                    break;
                case 'b':
                    context.window().display_line_at(
                        cursor.line, window.dimensions().line - 1);
                    break;
                case 'h':
...
{
    if (not CommandManager::has_instance()) return;
            ...
            if (context.has_client())
            {
                    ...
                    autoinfo & AutoInfo::Command)
                {
                    ...
                    if (not info.first.empty() and not info.second.empty())
                        context.client().info_show(info.first, info.second, {},
                                                   InfoStyle::Prompt);
                }
            }
...
template <bool replace> void pipe(Context &context, NormalParams)
{
    const char *prompt = replace ? "pipe:" : "pipe-to:";
    context.input_handler().prompt(
        prompt, "", get_face("Prompt"), false, shell_complete,
...
template <InsertMode mode> void insert_output(Context &context, NormalParams)
{
    const char *prompt =
                       ...
                       const Regex &regex, bool &main_wrapped)
{
    ...
    if (mode == SelectMode::Replace)
    {
        for (int i = 0; i < selections.size(); ++i)
        {
            ...
            sel = keep_direction(
                find_next_match<direction>(buffer, sel, regex, wrapped), sel);
            if (i == main_index) main_wrapped = wrapped;
        }
    }
    if (mode == SelectMode::Extend)
    {
        for (int i = 0; i < selections.size(); ++i)
        {
            auto &sel = selections[i];
            sel.merge_with(
                find_next_match<direction>(buffer, sel, regex, wrapped));
            if (i == main_index) main_wrapped = wrapped;
        }
    }
    ...
    {
        auto sel =
            keep_direction(find_next_match<direction>(buffer, selections.main(),
                                                      regex, main_wrapped),
                           selections.main());
...
constexpr InsertMode adapt_for_linewise(InsertMode mode)
{
    return ((mode == InsertMode::Append)
                ...
                : ((mode == InsertMode::Insert)
                       ...
                       : ((mode == InsertMode::Replace) ? InsertMode::Replace
                                                        : InsertMode::Insert)));
}

template <InsertMode mode> void paste(Context &context, NormalParams params)
{
    const char reg = params.reg ? params.reg : '"';
...
template <InsertMode mode> void paste_all(Context &context, NormalParams params)
{
    const char reg = params.reg ? params.reg : '"';
    auto strings = RegisterManager::instance()[reg].values(context);
    InsertMode effective_mode = mode;
...
{
    CharCoord position =
        context.has_window() ? context.window().position() : CharCoord{};
    SelectionList selections = context.selections();
    context.input_handler().prompt(
        prompt, "", get_face("Prompt"), false, complete_nothing,
        [=](StringView str, PromptEvent event, Context &context) mutable {
            try
            {
                ...
                if (event == PromptEvent::Abort or
                    (event == PromptEvent::Change and
                     (not incsearch or str.empty())))
                    return;

                if (event == PromptEvent::Validate) context.push_jump();
            ...
            catch (runtime_error &)
            {
                ...
                if (event == PromptEvent::Validate) throw;
            }
        });
}
    ...
    regex_prompt(context, direction == Forward ? "search:" : "reverse search:",
                 [reg, count](Regex ex, PromptEvent event, Context &context) {
                     ...
                     if (not ex.empty() and not ex.str().empty())
                     {
                         ...
                         do
                         {
                             select_next_match<direction, mode>(
                                 context.buffer(), context.selections(), ex,
                                 main_wrapped);
                         } while (--c > 0);
                     }
                 });
...
void search_next(Context &context, NormalParams params)
{
    ...
    if (not str.empty())
    {
        ...
        do
        {
            bool wrapped = false;
            select_next_match<direction, mode>(
                context.buffer(), context.selections(), ex, wrapped);
...
void split_lines(Context &context, NormalParams)
{
    ...
    for (auto &sel : selections)
    {
        ...
        res.push_back(keep_direction(
            {min, {min.line, buffer[min.line].length() - 1}}, sel));
        for (auto line = min.line + 1; line < max.line; ++line)
            res.push_back(
                keep_direction({line, {line, buffer[line].length() - 1}}, sel));
...
template <bool matching> void keep(Context &context, NormalParams)
{
    constexpr const char *prompt =
        matching ? "keep matching:" : "keep not matching:";
    regex_prompt(context, prompt, [](const Regex &ex, PromptEvent,
                                     Context &context) {
        if (ex.empty()) return;
        const Buffer &buffer = context.buffer();
        ...
        for (auto &sel : context.selections())
        {
            ...
            if (regex_search(begin, end, ex, RegexConstant::match_any) ==
                matching)
                keep.push_back(sel);
        }
        if (keep.empty()) throw runtime_error("no selections remaining");
...
template <bool indent_empty = false> void indent(Context &context, NormalParams)
{
...
void deindent(Context &context, NormalParams)
{
    ...
    CharCount indent_width = context.options()["indentwidth"].get<int>();
    if (indent_width == 0) indent_width = tabstop;
...
void select_object(Context &context, NormalParams params)
{
    ...
    on_next_key_with_autoinfo(
        ...
        [level](Key key, Context &context) {
            ...
            {
                Codepoint key;
                Selection (*func)(const Buffer &, const Selection &,
                                  ObjectFlags);
            } selectors[] = {
            ...
            {
                if (*cp == sel.key)
                    return select<mode>(context,
                                        std::bind(sel.func, _1, _2, flags));
            }

            if (*cp == 'u')
                return select<mode>(
                    context, std::bind(select_argument, _1, _2, level, flags));
            ...
            if (*cp == ':')
            {
                ...
                context.input_handler().prompt(
                    "object desc:", "", get_face("Prompt"), false,
                    complete_nothing,
                    [level, info](StringView cmdline, PromptEvent event,
                                  Context &context) {
                        ...
                        return select<mode>(
                            context,
                            std::bind(select_surrounding, _1, _2, params[0],
            ...
            {
                if (sur.opening[0_char] == *cp or sur.closing[0_char] == *cp or
                    (sur.name != 0 and sur.name == *cp))
                    return select<mode>(
                        context,
                        std::bind(select_surrounding, _1, _2, sur.opening,
                                  sur.closing, level, flags));
            }
        },
        get_title(),
        "b,(,):  parenthesis block\n"
...
void scroll(Context &context, NormalParams)
{
    ...
    auto cursor_pos =
        utf8::advance(buffer.iterator_at(position.line),
                      buffer.iterator_at(position.line + 1), position.column);
        ...
        for (int i = 0; i < std::max(params.count, 1); ++i)
        {
            ...
            if (anchor_byte != buffer[anchor_line].length() and
                ...
                result.emplace_back(ByteCoord{anchor_line, anchor_byte},
                                    ByteCoordAndTarget{cursor_line, cursor_byte,
                                                       cursor.target});
        }
    }
...
void rotate_selections_content(Context &context, NormalParams params)
{
    ...
    if (group == 0 or group > (int)strings.size()) group = (int)strings.size();
    count = count % group;
...
void replay_macro(Context &context, NormalParams params)
{
    ...
    ConstArrayView<String> reg_val =
        RegisterManager::instance()[reg].values(context);
...
template <Direction direction> void jump(Context &context, NormalParams)
{
    ...
    if (&buffer != oldbuf) context.change_buffer(buffer);
...
void align(Context &context, NormalParams)
{
    ...
    for (auto &sel : selections)
    {
        ...
        if (column >= columns.size()) columns.resize(column + 1);
    ...
    for (auto &col : columns)
    {
        ...
        for (auto &sel : col)
        {
            ...
            else
            {
                ...
                auto spaces =
                    targetcol - (tabs ? (tabcol + tabs * tabstop) : inscol);
...
void copy_indent(Context &context, NormalParams params)
{
    ...
    if (selection == 0) selection = context.selections().main_index() + 1;

    auto ref_line = selections[selection - 1].min().line;
    auto line = buffer[ref_line];
    auto it = line.begin();
    while (it != line.end() and is_horizontal_blank(*it)) ++it;
    const StringView indent = line.substr(0_byte, (int)(it - line.begin()));

    ScopedEdition edition{context};
    for (auto &l : lines)
    {
        if (l == ref_line) continue;

        auto line = buffer[l];
        ByteCount i = 0;
        while (i < line.length() and is_horizontal_blank(line[i])) ++i;
    ...
    for (auto &sel : context.selections())
    {
             ...
             it != end;)
        {
            if (*it == ' ')
            {
                ...
                if ((col % tabstop) == 0)
                    spaces.push_back(
                        {spaces_beg.coord(), (spaces_end - 1).coord()});
...
void save_selections(Context &context, NormalParams params)
{
    ...
    String desc =
        format("{}@{}%{}", selection_list_to_string(context.selections()),
               context.buffer().name(), context.buffer().timestamp());

    RegisterManager::instance()[reg] = desc;

    context.print_status({format("Saved selections in register '{}'", reg),
                          get_face("Information")});
}
...
void restore_selections(Context &context, NormalParams params)
{
    ...
    Buffer &buffer =
        BufferManager::instance().get_buffer({arobase + 1, percent});
    size_t timestamp = str_to_int({percent + 1, desc.end()});

    Vector<Selection> sels;
    for (auto sel_desc :
         StringView{desc.begin(), arobase} | split<StringView>(':'))
    ...
    else
    {
        ...
        sel_list.update();
        int main_index =
            sel_list.size() + context.selections_write_only().main_index();
        for (auto &sel : context.selections())
    ...
    context.print_status({format("Restored selections from register '{}'", reg),
                          get_face("Information")});
}
...
void exec_user_mappings(Context &context, NormalParams params)
{
    on_next_key_with_autoinfo(
        ...
        [params](Key key, Context &context) mutable {
            if (not context.keymaps().is_mapped(key, KeymapMode::User)) return;
            ...
            ScopedEdition edition(context);
            for (auto &key : mapping) context.input_handler().handle_key(key);
        },
...
void move(Context &context, NormalParams params)
{
    ...
    if (direction == Backward) offset = -offset;
    auto &selections = context.selections();
    for (auto &sel : selections)
    {
        auto cursor = context.has_window()
                          ? context.window().offset_coord(sel.cursor(), offset)
                          : context.buffer().offset_coord(sel.cursor(), offset);

        sel.anchor() = mode == SelectMode::Extend ? sel.anchor() : cursor;
        sel.cursor() = cursor;
    }
    if (std::is_same<Type, LineCount>::value) selections.avoid_eol();
...
{
    for (auto &sel : context.selections()) sel.anchor() = sel.cursor();
}
...
static NormalCmdDesc cmds[] = {
    ...
    {alt('R'), "replace selected text with yanked text",
    ...
    {'e', "select to next word end",
     repeated<select<SelectMode::Replace, select_to_next_word_end<Word>>>},
    {'b', "select to previous word start",
     repeated<select<SelectMode::Replace, select_to_previous_word<Word>>>},
    ...
    {'m', "select to matching character",
     select<SelectMode::Replace, select_matching>},
    {'M', "extend to matching character",
     select<SelectMode::Extend, select_matching>},
