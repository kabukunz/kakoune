          ...
          switch (to_lower(*cp)) {
            ...
            case 'b':
              if (context.has_window()) {
                ...
                auto line =
                    window.position().line + window.dimensions().line - 1;
                select_coord<mode>(buffer, line, context.selections());
              }
              break;
            case 'c':
              if (context.has_window()) {
                auto& window = context.window();
                auto line =
                    window.position().line + window.dimensions().line / 2;
                select_coord<mode>(buffer, line, context.selections());
              }
              break;
            case 'a': {
            ...
            case 'f': {
              ...
              if (contains_that(filename,
                                [](char c) { return contains(forbidden, c); }))
                return;

              auto paths = context.options()["path"]
      ...
      [count](Key key, Context& context) {
        ...
        switch (to_lower(*cp)) {
          ...
          case 't':
            ...
            break;
          case 'b':
            context.window().display_line_at(cursor.line,
      ...
      [params](StringView cmdline, PromptEvent event, Context& context) {
        if (context.has_client()) {
          ...
          if (event == PromptEvent::Change and autoinfo & AutoInfo::Command) {
            ...
            auto info =
                CommandManager::instance().command_info(context, cmdline);
      ...
      [](StringView cmdline, PromptEvent event, Context& context) {
        ...
        auto str =
            ShellManager::instance()
                .eval(real_cmd, context, {}, ShellManager::Flags::WaitForStdout)
                .first;
        ScopedEdition edition(context);
...
void select_next_match(const Buffer& buffer,
  ...
  regex_prompt(context, direction == Forward ? "search:" : "reverse search:",
               [reg, count](Regex ex, PromptEvent event, Context& context) {
                 ...
                 if (not ex.empty() and not ex.str().empty()) {
                   ...
                   int c = count;
                   do {
                     select_next_match<direction, mode>(context.buffer(),
                                                        context.selections(),
                                                        ex, main_wrapped);
                   } while (--c > 0);
                 }
...
void search_next(Context& context, NormalParams params) {
  ...
  if (not str.empty()) {
    ...
    do {
      bool wrapped = false;
      select_next_match<direction, mode>(context.buffer(), context.selections(),
                                         ex, wrapped);
...
void use_selection_as_search_pattern(Context& context, NormalParams params) {
  ...
  for (auto& sel : sels) {
    ...
    patterns.push_back(format(
        "{}\\Q{}\\E{}", smart and is_bow(buffer, beg) ? "\\b" : "",
        buffer.string(beg, end), smart and is_eow(buffer, end) ? "\\b" : ""));
  }
...
void split_lines(Context& context, NormalParams) {
  ...
  for (auto& sel : selections) {
    ...
    auto max = sel.max();
    res.push_back(
        keep_direction({min, {min.line, buffer[min.line].length() - 1}}, sel));
      ...
      [level](Key key, Context& context) {
        ...
        if (*cp == 'u')
          return select<mode>(context,
                              std::bind(select_argument, _1, _2, level, flags));
        ...
        if (*cp == ':') {
          ...
          context.input_handler().prompt(
              ...
              [level, info](StringView cmdline, PromptEvent event,
                ...
                return select<mode>(
                    context, std::bind(select_surrounding, _1, _2, params[0],
        ...
        for (auto& sur : surrounding_pairs) {
          if (sur.opening[0_char] == *cp or sur.closing[0_char] == *cp or
            ...
            return select<mode>(
                context, std::bind(select_surrounding, _1, _2, sur.opening,
  ...
  for (auto& sel : selections) {
    ...
    for (int i = 0; i < std::max(params.count, 1); ++i) {
      ...
      if (anchor_byte != buffer[anchor_line].length() and
        ...
        result.emplace_back(
            ByteCoord{anchor_line, anchor_byte},
...
template <>
struct WithBitOps<SelectFlags> : std::true_type {};
      ...
      [params](Key key, Context& context) {
        ...
        if (auto cp = key.codepoint())
          select<new_flags>(
              context,
              std::bind(
                  flags & SelectFlags::Reverse ? select_to_reverse : select_to,
