...
String to_json(Attribute attributes) {
  struct {
    ...
    StringView name;
      ...
      {Attribute::Exclusive, "exclusive"}, {Attribute::Underline, "underline"},
      {Attribute::Reverse, "reverse"},     {Attribute::Blink, "blink"},
      {Attribute::Bold, "bold"},           {Attribute::Dim, "dim"},
...
void JsonUI::info_show(StringView title,
                       StringView content,
                       CharCoord anchor,
                       Face face,
                       InfoStyle style) {
