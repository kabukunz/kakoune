...
String to_json(StringView str)
{
    ...
    for (auto it = str.begin(), end = str.end(); it != end;)
    {
        ...
        char buf[7] = {'\\', *next, 0};
        if (*next >= 0 and *next <= 0x1F) sprintf(buf, "\\u%04x", *next);
...
String to_json(Attribute attributes)
{
    ...
    for (auto &attr : attrs)
    {
        if (not(attributes & attr.attr)) continue;
...
{
    if (sizeof...(Args) != 0) return to_json(first) + ", " + concat(args...);
...
JsonUI::JsonUI()
    : m_stdin_watcher{0, [this](FDWatcher &,
                                EventMode mode) { parse_requests(mode); }},
      m_dimensions{24, 80}
{
...
bool JsonUI::is_key_available() { return not m_pending_keys.empty(); }
Key JsonUI::get_key()
{
...
void JsonUI::menu_show(ConstArrayView<DisplayLine> items, CharCoord anchor,
                       Face fg, Face bg, MenuStyle style)
{
    rpc_call("menu_show", items, anchor, fg, bg, style);
}

void JsonUI::menu_select(int selected) { rpc_call("menu_show", selected); }
void JsonUI::menu_hide() { rpc_call("menu_hide"); }
void JsonUI::info_show(StringView title, StringView content, CharCoord anchor,
...
CharCoord JsonUI::dimensions() { return m_dimensions; }
...
std::tuple<Value, const char *> parse_json(const char *pos, const char *end)
{
    ...
    if (not skip_while(pos, end, is_blank)) return {};
    ...
    {
        JsonArray array;
        if (*++pos == ']') return Result{std::move(array), pos + 1};
    ...
    {
        JsonObject object;
        if (*++pos == '}') return Result{std::move(object), pos + 1};

        while (true)
        {
            Value name_value;
            std::tie(name_value, pos) = parse_json(pos, end);
            if (not name_value) return {};

            String &name = name_value.as<String>();
            if (not skip_while(pos, end, is_blank)) return {};
            if (*pos++ != ':') throw runtime_error("expected :");
...
void JsonUI::parse_requests(EventMode mode)
{
    ...
    if (not m_requests.empty())
    {
        ...
        if (pos) m_requests = String{pos, m_requests.end()};
    }
