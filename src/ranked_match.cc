...
static bool subsequence_match_smart_case(StringView str,
                                         StringView subseq,
                                         int& index_sum) {
  index_sum = 0;
  auto it = str.begin();
...
RankedMatch::RankedMatch(StringView candidate,
                         StringView query,
                         TestFunc func) {
...
RankedMatch::RankedMatch(StringView candidate,
                         UsedLetters candidate_letters,
                         StringView query,
                         UsedLetters query_letters)
...
UnitTest test_ranked_match{[] {
  ...
  kak_assert(count_word_boundaries_match("countWordBoundariesMatch", "wm") ==
             2);
  kak_assert(count_word_boundaries_match("countWordBoundariesMatch", "cobm") ==
             3);
  kak_assert(count_word_boundaries_match("countWordBoundariesMatch", "cWBM") ==
             4);
