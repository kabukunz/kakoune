...
static int count_word_boundaries_match(StringView candidate, StringView query)
{
    ...
    for (Utf8It it{candidate.begin(), candidate}; it != candidate.end(); ++it)
    {
        ...
        const bool is_word_boundary =
            prev == 0 or (!iswalnum((wchar_t)prev) and iswalnum((wchar_t)c)) or
            (iswlower((wchar_t)prev) and iswupper((wchar_t)c));
        prev = c;

        if (not is_word_boundary) continue;
...
{
    if (candidate.empty() or query.length() > candidate.length()) return;

    if (query.empty())
        m_candidate = candidate;
    else if (func() and
             subsequence_match_smart_case(candidate, query, m_match_index_sum))
    {
        m_candidate = candidate;

        m_first_char_match = smartcase_eq(query[0], candidate[0]);
        m_word_boundary_match_count =
            count_word_boundaries_match(candidate, query);
...
bool RankedMatch::operator<(const RankedMatch &other) const
{
    ...
    if (m_prefix != other.m_prefix) return m_prefix;
    ...
    if (m_only_word_boundary and other.m_only_word_boundary)
    {
        if (m_word_boundary_match_count != other.m_word_boundary_match_count)
            return m_word_boundary_match_count >
                   other.m_word_boundary_match_count;
    }
    ...
    Utf8It it1{m_candidate.begin(), m_candidate},
        it2{other.m_candidate.begin(), other.m_candidate};
    for (; it1 != m_candidate.end() and it2 != other.m_candidate.end();
         ++it1, ++it2)
    {
...
UnitTest test_ranked_match{[] {
    ...
    kak_assert(
        not(RankedMatch{"source_data", "so"} < RankedMatch{"source", "so"}));
