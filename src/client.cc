...
Client::Client(std::unique_ptr<UserInterface> &&ui,
               std::unique_ptr<Window> &&window, SelectionList selections,
               EnvVarMap env_vars, String name)
    : m_ui{std::move(ui)},
      ...
      m_env_vars(env_vars)
{
    ...
    m_ui->set_input_callback(
        [this](EventMode mode) { handle_available_input(mode); });
    force_redraw();
}
        ...
        try
        {
            while (Optional<Key> key = get_next_key(mode))
            {
                if (*key == ctrl('c'))
                    ...
                    context().hooks().run_hook("FocusIn", context().name(),
                                               context());
                else if (*key == Key::FocusOut)
                    context().hooks().run_hook("FocusOut", context().name(),
                                               context());
...
DisplayLine Client::generate_mode_line() const
{
    ...
    catch (runtime_error &err)
    {
        ...
        modeline.push_back(
            {"modelinefmt error, see *debug* buffer", get_face("Error")});
    }
    ...
    if (m_input_handler.is_recording())
        modeline.push_back(
            {format("[recording ({})]", m_input_handler.recording_reg()),
             info_face});
    ...
    modeline.push_back(
        {format(" - {}@[{}]", context().name(), Server::instance().session())});
...
{
    if (m_buffer_reload_dialog_opened) close_buffer_reload_dialog();
...
void Client::redraw_ifn()
{
    ...
    if (window.needs_redraw(context())) m_ui_pending |= Draw;
    ...
    if (m_ui_pending & MenuShow)
    {
        m_menu.ui_anchor = m_menu.style == MenuStyle::Inline
                               ? window.display_position(m_menu.anchor)
                               : CharCoord{};
        m_ui->menu_show(m_menu.items, m_menu.ui_anchor,
                        get_face("MenuForeground"), get_face("MenuBackground"),
                        m_menu.style);
    }
    if (m_ui_pending & MenuSelect) m_ui->menu_select(m_menu.selected);
    if (m_ui_pending & MenuHide) m_ui->menu_hide();
    ...
    if (m_ui_pending & InfoHide) m_ui->info_hide();
...
{
    if (m_buffer_reload_dialog_opened) return;
    ...
    if (ts == InvalidTime or ts == buffer.fs_timestamp()) return;
...
void Client::menu_show(Vector<DisplayLine> choices, ByteCoord anchor,
                       MenuStyle style)
{
...
void Client::info_show(String title, String content, ByteCoord anchor,
                       InfoStyle style)
{
