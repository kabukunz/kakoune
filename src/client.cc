...
void Client::redraw_ifn() {
  ...
  if (m_ui_pending & MenuShow) {
    m_menu.ui_anchor = m_menu.style == MenuStyle::Inline
                           ...
                           : CharCoord{};
    m_ui->menu_show(m_menu.items, m_menu.ui_anchor, get_face("MenuForeground"),
                    get_face("MenuBackground"), m_menu.style);
  }
...
void Client::on_buffer_reload_key(Key key) {
  ...
  if (key == 'y' or key == ctrl('m'))
    ...
    print_status(
        {format("'{}' kept", buffer.display_name()), get_face("Information")});
  } else {
    print_status({format("'{}' is not a valid choice", key_to_str(key)),
                  get_face("Error")});
    m_input_handler.on_next_key(KeymapMode::None, [this](Key key, Context&) {
...
void Client::menu_show(Vector<DisplayLine> choices,
                       ByteCoord anchor,
                       MenuStyle style) {
...
void Client::info_show(String title,
                       String content,
                       ByteCoord anchor,
                       InfoStyle style) {
