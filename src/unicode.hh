...
inline bool is_eol(Codepoint c) { return c == '\n'; }
inline bool is_horizontal_blank(Codepoint c) { return c == ' ' or c == '\t'; }
inline bool is_blank(Codepoint c) { return c == ' ' or c == '\t' or c == '\n'; }
enum WordType
{
    Word,
    WORD
...
template <WordType word_type = Word> inline bool is_word(Codepoint c)
{
    return c == '_' or iswalnum((wchar_t)c);
}

template <> inline bool is_word<WORD>(Codepoint c)
{
...
{
    if (is_eol(c)) return CharCategories::EndOfLine;
    if (is_horizontal_blank(c)) return CharCategories::Blank;
    if (word_type == WORD or is_word(c)) return CharCategories::Word;
