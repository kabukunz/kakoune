...
template <>
struct WithBitOps<OptionFlags> : std::true_type {};
...
class TypedOption : public Option {
 public:
  ...
  String get_as_string() const override { return option_to_string(m_value); }
...
const T& Option::get() const {
  ...
  if (not typed_opt)
    throw runtime_error(
        format("option '{}' is not of type '{}'", name(), typeid(T).name()));
...
auto find_option(T& container, StringView name) -> decltype(container.begin()) {
  ...
  return find_if(container,
                 [&name](const ptr_type& opt) { return opt->name() == name; });
}
...
class OptionsRegistry {
 public:
  ...
  Option& declare_option(const String& name,
    ...
    if (it != opts.end()) {
      ...
      throw runtime_error(format(
          "option '{}' already declared with different type or flags", name));
    }
    m_descs.emplace_back(new OptionDesc{name, docstring, flags});
    opts.emplace_back(new TypedCheckedOption<T, validator>{
        m_global_manager, *m_descs.back(), value});
    return *opts.back();
  }

  const OptionDesc* option_desc(StringView name) const {
    auto it = find_if(m_descs, [&name](const std::unique_ptr<OptionDesc>& opt) {
