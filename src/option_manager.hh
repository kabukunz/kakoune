   ...
   public:
    ...
    void set(T value, bool notify = true)
    {
        ...
        if (m_value != value)
        {
            ...
            if (notify) manager().on_option_changed(*this);
        }
    }
    ...
    {
        if (option_add(m_value, str)) m_manager.on_option_changed(*this);
    }
...
auto find_option(T &container, StringView name) -> decltype(container.begin())
{
    ...
    return find_if(container, [&name](const ptr_type &opt) {
...
class OptionsRegistry
{
   public:
                           ...
                           OptionFlags flags = OptionFlags::None)
    {
        ...
        if (it != opts.end())
        {
            if ((*it)->is_of_type<T>() and (*it)->flags() == flags) return **it;
    ...
    {
        auto it =
            find_if(m_descs, [&name](const std::unique_ptr<OptionDesc> &opt) {
                return opt->name() == name;
            });
