...
class SafeCountable
{
...
#ifdef SAFE_PTR_TRACK_CALLSTACKS
        auto it =
            std::find_if(sc->m_callstacks.begin(), sc->m_callstacks.end(),
   ...
   private:
    ...
    mutable int m_count;
#else
    [[gnu::always_inline]] friend void inc_ref_count(const SafeCountable *sc,
                                                     void *ptr){}

        [[gnu::always_inline]] friend void dec_ref_count(
            const SafeCountable *sc, void *ptr)
    {
    }
#endif
};
...
using PropagateConst =
    typename std::conditional<std::is_const<T>::value, const U, U>::type;
