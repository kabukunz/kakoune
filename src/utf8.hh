...
[[gnu::always_inline]] inline char read(Iterator &it)
{
    char c = *it;
    ++it;
    return c;
...
template <typename Iterator> Iterator next(Iterator it, const Iterator &end)
{
...
template <typename Iterator> Iterator finish(Iterator it, const Iterator &end)
{
    while (it != end and (*(it)&0xC0) == 0x80) ++it;
    return it;
}
...
Iterator advance(Iterator it, const Iterator &end, CharCount d)
{
    if (d < 0)
    {
        while (it != end and d++) it = utf8::previous(it, end);
    }
    else
    {
        while (it != end and d--) it = utf8::next(it, end);
    }
    return it;
}
...
// multibyte) character
inline bool is_character_start(char c) { return (c & 0xC0) != 0x80; }
...
CharCount distance(Iterator begin, const Iterator &end)
{
    ...
    while (begin != end)
    {
        if (is_character_start(*begin++)) ++dist;
    }
    return dist;
}
...
{
    while (it != begin and not is_character_start(*it)) --it;
    return it;
}
...
{
    if (it == end) return InvalidPolicy{}(-1);
    ...
    if (it == end) return InvalidPolicy{}(byte);
    ...
    if ((byte & 0xF0) == 0xE0)  // 1110xxxx
    {
        ...
        if (it == end) return InvalidPolicy{}(cp);
...
template <typename OutputIterator> void dump(OutputIterator &&it, Codepoint cp)
{
    if (cp <= 0x7F)
        *it++ = cp;
    else if (cp <= 0x7FF)
