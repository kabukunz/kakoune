            ...
            if (auto cp = key.codepoint()) {
              if (*cp <= 127)
                ...
                context.print_status(
                    {format("invalid register '{}'", *cp), get_face("Error")});
            }
          },
 ...
 public:
  void handle_key(Key key) {
    if (key == Key::Left or key == alt('h')) {
      ...
      if (m_cursor_pos != 0) {
        m_line =
            m_line.substr(0, m_cursor_pos - 1) + m_line.substr(m_cursor_pos);
      ...
      if (m_cursor_pos != m_line.char_length())
        m_line =
            m_line.substr(0, m_cursor_pos) + m_line.substr(m_cursor_pos + 1);
    } else if (key == ctrl('w'))
  ...
  DisplayLine build_display_line(CharCount width) {
    ...
    if (m_cursor_pos == m_line.char_length())
      return DisplayLine{{{cleanup(m_line.substr(m_display_pos, width - 1)),
                           get_face("StatusLine")},
                          {" "_str, get_face("StatusCursor")}}};
    else
      return DisplayLine(
          {{cleanup(m_line.substr(m_display_pos, m_cursor_pos - m_display_pos)),
            get_face("StatusLine")},
...
class Menu : public InputMode {
 public:
  Menu(InputHandler& input_handler,
...
class Prompt : public InputMode {
 public:
  Prompt(InputHandler& input_handler,
         StringView prompt,
         String initstr,
         Face face,
         bool password,
...
class NextKey : public InputMode {
 public:
  NextKey(InputHandler& input_handler,
...
class Insert : public InputMode {
 public:
  ...
  void on_disabled() override { m_idle_timer.set_next_date(TimePoint::max()); }
...
InputHandler::InputHandler(SelectionList selections,
                           Context::Flags flags,
                           String name)
...
void InputHandler::prompt(StringView prompt,
                          String initstr,
                          Face prompt_face,
                          bool password,
                          Completer completer,
...
bool show_auto_info_ifn(StringView title,
                        StringView info,
                        AutoInfo mask,
