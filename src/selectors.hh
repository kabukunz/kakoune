...
Selection select_to_next_word(const Buffer &buffer, const Selection &selection)
{
    ...
    if (begin + 1 == buffer.end()) return selection;
...
{
    Utf8Iterator begin{buffer.iterator_at(selection.cursor()), buffer};
    if (begin == buffer.begin()) return selection;
    if (categorize<word_type>(*begin) != categorize<word_type>(*(begin - 1)))
        --begin;
                      ...
                      ObjectFlags flags)
{
    ...
    if (is_word<word_type>(*first))
    {
        if (flags & ObjectFlags::ToBegin)
        {
            ...
            if (not is_word<word_type>(*first)) ++first;
        }
    ...
    else if (not(flags & ObjectFlags::Inner))
    {
        if (flags & ObjectFlags::ToBegin)
        {
            ...
            if (not is_word<word_type>(*first)) return selection;
            skip_while_reverse(first, buffer.begin(), is_word<word_type>);
            if (not is_word<word_type>(*first)) ++first;
        }
        if (flags & ObjectFlags::ToEnd)
                            ...
                            const Regex &regex)
{
                                    ...
                                    is_bow(buffer, begin.coord()), is_pos_eow)))
    {
        ...
        if (res.empty() or matches[0].second > res[0].second) res.swap(matches);
    }
                          ...
                          const Regex &ex, bool &wrapped)
{
    ...
    {
        if (find_last_match(buffer, pos, matches, ex)) return true;
        wrapped = true;
                          ...
                          const Regex &regex, bool &wrapped)
{
    ...
    if (direction == Backward) std::swap(begin, end);
