...
void CommandManager::register_command(String command_name, Command command,
                                      String docstring,
                                      ParameterDesc param_desc,
                                      CommandFlags flags, CommandHelper helper,
                                      CommandCompleter completer)
{
    m_commands[command_name] = {std::move(command),    std::move(docstring),
...
{
   public:
    [[gnu::always_inline]] char operator*() const { return str[pos]; }
    Reader &operator++()
    {
        if (str[pos++] == '\n')
        {
    ...
    [[gnu::always_inline]] explicit operator bool() const
    {
        return pos < str.length();
    }

    [[gnu::always_inline]] StringView substr_from(ByteCount start) const
    {
...
String get_until_delimiter(Reader &reader, Func is_delimiter)
{
    ...
    if (beg < reader.str.length()) str += reader.substr_from(beg);
    return str;
}

[[gnu::always_inline]] inline String get_until_delimiter(Reader &reader, char c)
{
...
template <bool throw_on_invalid> Token::Type token_type(StringView type_name)
{
    if (type_name == "")
...
template <bool throw_on_unterminated> Token parse_percent_token(Reader &reader)
{
    ++reader;
    const ByteCount type_start = reader.pos;
    while (reader and isalpha(*reader)) ++reader;
...
template <bool throw_on_unterminated> TokenList parse(StringView line)
{
    TokenList result;
    ...
    while (reader)
    {
        ...
        if (c == '"' or c == '\'')
        {
            ...
            result.emplace_back(
                c == '"' ? Token::Type::RawEval : Token::Type::RawQuoted, start,
                reader.pos, coord, std::move(token));
        }
        else if (c == '%')
            result.push_back(
        ...
        else
        {
            ...
            if (not str.empty())
                result.emplace_back(Token::Type::Raw, start, reader.pos, coord,
                                    unescape(str, "%", '\\'));
        }

        if (is_command_separator(*reader))
            result.emplace_back(Token::Type::CommandSeparator, reader.pos,
                                reader.pos + 1, coord);
...
CommandManager::CommandMap::const_iterator CommandManager::find_command(
    const Context &context, const String &name) const
{
                                            ...
                                            CharCoord pos) const
{
    ...
    if (command_it == m_commands.end()) throw command_not_found(params[0]);
    ...
    {
        throw runtime_error(format("{}:{}: '{}' {}", pos.line + 1,
                                   pos.column + 1, command_it->first,
                                   error.what()));
    }
}
                             ...
                             const ShellContext &shell_context)
{
    ...
    {
        if (params.empty()) command_coord = it->coord();
        ...
        {
            auto shell_tokens =
                parse<true>(expand_token(*it, context, shell_context));
            it = tokens.erase(it);
...
CommandInfo CommandManager::command_info(const Context &context,
                                         StringView command_line) const
{
    ...
    {
        if (tokens[i].type() == Token::Type::CommandSeparator) cmd_idx = i + 1;
    }
    ...
    if (cmd == m_commands.end()) return res;
    ...
    for (auto &alias : context.aliases().aliases_for(cmd->first))
        aliases += " " + alias;
    if (not aliases.empty()) res.second += "Aliases:" + aliases + "\n";
...
{
    auto commands = m_commands | filter([](const CommandMap::value_type &cmd) {
                        return not(cmd.second.flags & CommandFlags::Hidden);
                    }) |
                    transform([](const CommandMap::value_type &cmd) {
                        return StringView{cmd.first};
                    });

    if (not with_aliases)
        return {0, query.length(),
                Kakoune::complete(query, query.length(), commands)};
                                     ...
                                     ByteCount cursor_pos)
{
    ...
    if (tokens.empty() or
        (tok_idx == cmd_idx and
         (is_last_token or tokens[tok_idx].type() == Token::Type::Raw or
          tokens[tok_idx].type() == Token::Type::RawQuoted)))
    {
        auto cmd_start = is_last_token ? cursor_pos : tokens[tok_idx].begin();
        StringView query =
            command_line.substr(cmd_start, cursor_pos - cmd_start);
        return offset_pos(complete_command_name(context, query, true),
                          cmd_start);
    }
    ...
    ByteCount start =
        tok_idx < tokens.size() ? tokens[tok_idx].begin() : cursor_pos;
    ByteCount cursor_pos_in_token = cursor_pos - start;

    const Token::Type type =
        tok_idx < tokens.size() ? tokens[tok_idx].type() : Token::Type::Raw;
    switch (type)
    {
        case Token::Type::OptionExpand:
            return {
        ...
        case Token::Type::ShellExpand:
            return offset_pos(
                shell_complete(context, flags, tokens[tok_idx].content(),
                               cursor_pos_in_token),
                start);
        ...
        case Token::Type::RawEval:
        {
            ...
            Vector<String> params;
            for (auto it = tokens.begin() + cmd_idx + 1; it != tokens.end();
                 ++it)
                params.push_back(it->content());
            if (tok_idx == tokens.size()) params.push_back("");
            Completions completions =
                offset_pos(command_it->second.completer(context, flags, params,
                                                        tok_idx - cmd_idx - 1,
                                                        cursor_pos_in_token),
                           start);
...
Completions PerArgumentCommandCompleter::operator()(
    const Context &context, CompletionFlags flags, CommandParameters params,
    size_t token_to_complete, ByteCount pos_in_token) const
{
    if (token_to_complete >= m_completers.size()) return Completions{};
