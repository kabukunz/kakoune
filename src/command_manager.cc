...
[[gnu::always_inline]] inline String get_until_delimiter(Reader& reader,
                                                         char c) {
  return get_until_delimiter(reader, [c](char ch) { return c == ch; });
}

StringView get_until_closing_delimiter(Reader& reader,
...
String expand_token(const Token& token,
  ...
  switch (token.type()) {
    case Token::Type::ShellExpand:
      return ShellManager::instance()
          .eval(content, context, {}, ShellManager::Flags::WaitForStdout,
                shell_context)
          .first;
...
TokenList parse(StringView line) {
  ...
  while (reader) {
    ...
    if (c == '"' or c == '\'') {
      ...
      result.push_back(parse_percent_token<throw_on_unterminated>(reader));
    else {
...
Completions CommandManager::complete_command_name(const Context& context,
  ...
  auto candidates = Kakoune::complete(
      query, query.length(),
      concatenated(commands, context.aliases().flatten_aliases() |
...
Completions CommandManager::complete(const Context& context,
  ...
  if (token_to_complete == 0)
    ...
    if (command_it != m_commands.end() and command_it->second.completer)
      return command_it->second.completer(context, flags,
                                          params.subrange(1, params.size() - 1),
...
Completions PerArgumentCommandCompleter::operator()(
  ...
  const String& argument =
      token_to_complete < params.size() ? params[token_to_complete] : String();
