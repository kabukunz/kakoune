...
template <typename Iterator, typename CodepointType = Codepoint,
                                      ...
                                      CodepointType, DifferenceType>
{
   public:
    ...
    {
        if (count < 0) return operator-(-count);
    ...
    CodepointType operator*() const { return get_value(); }
   ...
   private:
    ...
    CodepointType get_value() const
    {
        if (m_value == (CodepointType)-1)
            m_value =
                (CodepointType)utf8::codepoint<InvalidPolicy>(m_it, m_end);
        return m_value;
    }
