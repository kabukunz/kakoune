...
// a wrapping class
template <typename T> class OnScopeEnd
{
   public:
...
template <typename T> OnScopeEnd<T> on_scope_end(T t)
{
...
template <typename T> bool operator==(const std::unique_ptr<T> &lhs, T *rhs)
{
    return lhs.get() == rhs;
}
...
{
    while (it != end and condition(*it)) ++it;
    return it != end;
}
...
{
    while (it != begin and condition(*it)) --it;
    return condition(*it);
}
}
