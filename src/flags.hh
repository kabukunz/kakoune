...
namespace Kakoune
{
template <typename Flags> struct WithBitOps : std::false_type
{
};
...
using EnableIfWithBitOps =
    typename std::enable_if<WithBitOps<Flags>::value, T>::type;

template <typename Flags, typename T = void>
using EnableIfWithoutBitOps =
    typename std::enable_if<not WithBitOps<Flags>::value, T>::type;
