...
static Face parse_face(StringView facedesc)
{
    ...
    if (attr_it != facedesc.end())
    {
        for (++attr_it; attr_it != facedesc.end(); ++attr_it)
        {
            ...
            {
                case 'e':
                    res.attributes |= Attribute::Exclusive;
                    break;
                case 'u':
                ...
                case 'i':
                    ...
                    break;
                default:
                    throw runtime_error(format("unknown face attribute '{}'",
                                               StringView{*attr_it}));
            }
        }
    }
...
Face FaceRegistry::operator[](const String &facedesc)
{
    ...
    {
        if (it->second.alias.empty()) return it->second.face;
                                  ...
                                  bool override)
{
    ...
    if (it != m_aliases.end())
    {
        while (it != m_aliases.end())
        {
            if (it->second.alias.empty()) break;
                                                ...
                                                ByteCount cursor_pos) const
{
    return complete(
        prefix, cursor_pos,
        m_aliases |
            transform([](const AliasMap::value_type &v) -> const String & {
                return v.first;
            }));
}
