...
void register_env_vars()
{
    ...
    for (auto &env_var : env_vars)
        shell_manager.register_env_var(env_var.name, env_var.prefix,
                                       env_var.func);
}
...
void register_registers()
{
    ...
    {
        register_manager.add_register(
            '0' + i, make_dyn_reg([i](const Context &context) {
                StringList result;
...
void register_options()
{
    ...
    reg.declare_option<CharCoord, check_scrolloff>(
        "scrolloff",
        "number of lines and columns to keep visible main cursor "
        "when scrolling",
        {0, 0});
    reg.declare_option("eolformat", "end of line format: crlf or lf",
                       EolFormat::Lf);
    reg.declare_option(
        "BOM", "insert a byte order mark when writing buffer (none or utf8)",
        ByteOrderMark::None);
    reg.declare_option("incsearch",
                       "incrementaly apply search/select/split regex", true);
    ...
    reg.declare_option("autoshowcompl",
                       ...
                       true);
    reg.declare_option(
        "aligntab", "use tab characters when possible for alignement", false);
...
std::unique_ptr<UserInterface> make_ui(UIType ui_type)
{
    struct DummyUI : UserInterface
    {
        ...
        void menu_show(ConstArrayView<DisplayLine>, CharCoord, Face, Face,
                       MenuStyle) override
        {
        }
        void menu_select(int) override {}
        void menu_hide() override {}
        void info_show(StringView, StringView, CharCoord, Face,
                       InfoStyle) override
        {
        }
    ...
    {
        case UIType::NCurses:
            return make_unique<NCursesUI>();
        case UIType::Json:
            return make_unique<JsonUI>();
        case UIType::Dummy:
...
{
    if (ui_type != UIType::NCurses) return make_ui(ui_type);
    ...
    if (not isatty(1)) throw startup_error("stdout is not a tty");
...
void signal_handler(int signal)
{
    ...
    if (Server::has_instance()) Server::instance().close_session();
...
int run_client(StringView session, StringView init_command, UIType ui_type)
{
    try
    {
        ...
        RemoteClient client{session, make_ui(ui_type), get_env_vars(),
                            init_command};
        while (true) event_manager.handle_next_events(EventMode::Normal);
    }
...
int run_server(StringView session, StringView init_command, bool ignore_kakrc,
               bool daemon, UIType ui_type, ConstArrayView<StringView> files,
    ...
    if (not ignore_kakrc) try
        {
            ...
            command_manager.execute(
                format("source {}/kakrc", runtime_directory()),
...
int run_filter(StringView keystr, StringView commands,
               ConstArrayView<StringView> files, bool quiet)
{
    StringRegistry string_registry;
    GlobalScope global_scope;
    EventManager event_manager;
    ...
    try
    {
        ...
        auto apply_to_buffer = [&](Buffer &buffer) {
            try
            {
                ...
                for (auto &key : keys) input_handler.handle_key(key);
            }
...
int main(int argc, char *argv[])
{
    ...
    for (size_t i = 1; i < argc; ++i) params.push_back(argv[i]);
    ...
    try
    {
        ...
        if (list_sessions or clear_sessions)
        {
            ...
            for (auto &session :
                 list_files(format("/tmp/kakoune/{}/", username)))
            {
                ...
                if (not valid and clear_sessions)
                {
                    ...
                    format_to(socket_file, "/tmp/kakoune/{}/{}", username,
                              session);
                    unlink(socket_file);
                }
            }
        ...
        if (auto session = parser.get_switch("p"))
        {
            for (auto opt : {"c", "n", "s", "d", "e"})
            {
                ...
                {
                    write_stderr(
                        format("error: -{} makes not sense with -p\n", opt));
                    return -1;
                }
            }
        ...
        if (auto server_session = parser.get_switch("c"))
        {
            for (auto opt : {"n", "s", "d"})
            {
                ...
                {
                    write_stderr(
                        format("error: -{} makes not sense with -c\n", opt));
                    return -1;
                }
            }
            String new_files;
        ...
        else
        {
            ...
            {
                return run_server(
                    session, init_command, (bool)parser.get_switch("n"),
                    (bool)parser.get_switch("d"), ui_type, files, target_coord);
            }
            catch (convert_to_client_mode &convert)
            {
                ...
                return run_client(
                    convert.session,
                    format("try %^buffer '{}'^; echo converted to client only "
                           "mode",
