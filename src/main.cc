...
OptionsRegistry& register_options() {
  ...
  reg.declare_option<int, check_tabstop>("tabstop", "size of a tab character",
                                         8);
  reg.declare_option<int, check_indentwidth>("indentwidth", "indentation width",
                                             4);
  ...
  reg.declare_option("aligntab",
                     "use tab characters when possible for alignement", false);
...
std::unique_ptr<UserInterface> make_ui(UIType ui_type) {
  struct DummyUI : UserInterface {
    ...
    void menu_show(ConstArrayView<DisplayLine>,
                   CharCoord,
                   Face,
                   Face,
                   MenuStyle) override {}
    ...
    void draw_status(const DisplayLine&,
                     const DisplayLine&,
                     const Face&) override {}
...
int run_server(StringView session,
  ...
  if (not files.empty())
    try {
      ...
      write_to_debug_buffer(
          format("error while opening command line files: {}", error.what()));
    }
  else
    buffer_manager.create_buffer("*scratch*", Buffer::Flags::None);

  if (not daemon) {
    local_client = client_manager.create_client(create_local_ui(ui_type),
...
int run_filter(StringView keystr,
               StringView commands,
               ConstArrayView<StringView> files,
               bool quiet) {
  StringRegistry string_registry;
...
int cli(int argc, char* argv[]) {
  ...
  try {
    ...
    UIType ui_type;
    if (ui_name == "ncurses")
      ui_type = UIType::NCurses;
    else if (ui_name == "json")
      ui_type = UIType::Json;
    else if (ui_name == "dummy")
      ui_type = UIType::Dummy;
    else {
