...
String ClientManager::generate_name() const
{
    for (int i = 0; true; ++i)
    {
        ...
        if (validate_client_name(name)) return name;
    }
}
                                     ...
                                     StringView init_commands)
{
    ...
    try
    {
        try
        {
            CommandManager::instance().execute(init_commands,
                                               client->context());
        }
        catch (Kakoune::runtime_error &error)
        {
            client->context().print_status(
                {error.what().str(), get_face("Error")});
...
{
    auto it = find_if(m_clients, [&](const std::unique_ptr<Client> &ptr) {
                                    ...
                                    SelectionList selections)
{
    ...
    m_free_windows.push_back({std::move(window),
...
void ClientManager::ensure_no_client_uses_buffer(Buffer &buffer)
{
    for (auto &client : m_clients)
    {
        ...
        if (client->last_buffer() == &buffer) client->set_last_buffer(nullptr);

        if (&client->context().buffer() != &buffer) continue;

        if (client->context().is_editing())
            throw runtime_error(
                format("client '{}' is inserting in buffer '{}'",
                       client->context().name(), buffer.display_name()));
...
Client *ClientManager::get_client_ifp(StringView name)
{
    for (auto &client : m_clients)
    {
        if (client->context().name() == name) return client.get();
    }
    return nullptr;
}
...
{
    if (Client *client = get_client_ifp(name)) return *client;
    throw runtime_error(format("no client named '{}'", name));
}

void ClientManager::redraw_clients() const
{
    for (auto &client : m_clients) client->redraw_ifn();
}
