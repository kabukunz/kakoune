...
static constexpr StringView assistant_cat[] = {R"(  ___            )",
                                               R"( (__ \           )",
                                               R"(   / /          ╭)",
                                               R"(  .' '·.        │)",
                                               R"( '      ”       │)",
...
void NCursesUI::set_face(NCursesWin* window,
                         Face face,
...
void NCursesUI::draw_line(NCursesWin* window,
...
Key NCursesUI::get_key() {
  ...
  if (c == KEY_MOUSE) {
    ...
    if (getmouse(&ev) == OK) {
      ...
      if (BUTTON_PRESS(ev.bstate, 1))
        return mouse_press(pos);
      if (BUTTON_RELEASE(ev.bstate, 1))
        return mouse_release(pos);
...
void NCursesUI::draw_menu() {
  ...
  for (auto line = 0_line; line < win_height; ++line) {
    ...
    for (int col = 0; col < m_menu.columns; ++col) {
      const int item_idx = (int)(m_menu.top_line + line) * m_menu.columns + col;
...
static CharCoord compute_pos(CharCoord anchor,
                             CharCoord size,
                             NCursesUI::Rect rect,
                             NCursesUI::Rect to_avoid,
                             bool prefer_above) {
  CharCoord pos;
  if (prefer_above) {
...
String make_info_box(StringView title,
                     StringView message,
                     CharCount max_width,
...
void NCursesUI::info_show(StringView title,
                          StringView content,
                          CharCoord anchor,
                          Face face,
                          InfoStyle style) {
