...
Snake find_end_snake_of_further_reaching_dpath(Iterator a,
                                               int N,
                                               Iterator b,
                                               int M,
...
template <typename Iterator, typename Equal>
SnakeLen find_middle_snake(Iterator a,
                           int N,
                           Iterator b,
                           int M,
                           ArrayView<int> data1,
  ...
  for (int D = 0; D <= (M + N + 1) / 2; ++D) {
    for (int k1 = -D; k1 <= D; k1 += 2) {
      auto p =
          find_end_snake_of_further_reaching_dpath(a, N, b, M, V1, D, k1, eq);
      V1[k1] = p.u;
    ...
    for (int k2 = -D; k2 <= D; k2 += 2) {
      auto p =
          find_end_snake_of_further_reaching_dpath(ra, N, rb, M, V2, D, k2, eq);
      V2[k2] = p.u;
...
void find_diff_rec(Iterator a,
  ...
  if (lenA > 0 and lenB > 0) {
    ...
    if (middle_snake.d > 1) {
      find_diff_rec(a, offA, middle_snake.x, b, offB, middle_snake.y, data1,
                    data2, eq, diffs);
      ...
      find_diff_rec(a, offA + middle_snake.u, lenA - middle_snake.u, b,
                    offB + middle_snake.v, lenB - middle_snake.v, data1, data2,
                    eq, diffs);
    } else {
      if (middle_snake.d == 1) {
...
Vector<Diff> find_diff(Iterator a,
                       int N,
                       Iterator b,
                       int M,
                       Equal eq = Equal{}) {
