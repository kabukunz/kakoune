...
{
    if (*this == ctrl('m')) return '\n';
    if (*this == ctrl('i')) return '\t';
...
String key_to_str(Key key)
{
    ...
    if (named) res = StringView{'<'} + res + StringView{'>'};
    return res;
}
...
UnitTest test_keys{[]() {
    ...
    String keys_as_str;
    for (auto &key : keys) keys_as_str += key_to_str(key);
