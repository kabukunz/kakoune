...
{
    if (not has_buffer()) throw runtime_error("no buffer in context");
    return const_cast<Buffer &>((*m_selections).buffer());
}

Window &Context::window() const
{
    if (not has_window()) throw runtime_error("no window in context");
    return *m_window;
}
...
{
    if (not has_client()) throw runtime_error("no client in context");
    return *m_client;
}
...
{
    if (has_client()) client().print_status(std::move(status));
}
...
{
    if (m_current != m_jumps.size() and m_current + 1 != m_jumps.size())
    {
...
const SelectionList &JumpList::backward(const SelectionList &current)
{
    ...
    if (m_current != 0)
    {
        ...
        {
            push(current);
            if (--m_current == 0) throw runtime_error("no previous jump");
        }
        SelectionList &res = m_jumps[--m_current];
        res.update();
        return res;
...
void Context::change_buffer(Buffer &buffer)
{
    ...
    if (m_edition_level > 0) this->buffer().commit_undo_group();
...
{
    if (not m_selections) throw runtime_error("no selections in context");
    (*m_selections).update();
    return *m_selections;
}
...
{
    if (m_edition_level >= 0) ++m_edition_level;
}

void Context::end_edition()
{
    if (m_edition_level < 0) return;

    kak_assert(m_edition_level != 0);
    if (m_edition_level == 1) buffer().commit_undo_group();
...
StringView Context::main_sel_register_value(StringView reg) const
{
    ...
    if (strings.size() <= index) index = strings.size() - 1;
