...
Selection select_matching(const Buffer &buffer, const Selection &selection)
{
    ...
    while (not is_eol(*it))
    {
        ...
        if (match != matching_pairs.end()) break;
        ++it;
    }
    if (match == matching_pairs.end()) return selection;
...
Optional<Iterator> find_closing(Iterator pos, Iterator end, Container opening,
                                Container closing, int init_level,
                                bool nestable)
{
...
Optional<std::pair<Iterator, Iterator>> find_surrounding(
    Iterator begin, Iterator end, Iterator pos, StringView opening,
    StringView closing, ObjectFlags flags, int init_level)
{
    ...
    if (to_begin)
    {
        ...
        auto res =
            find_closing(RevIt{pos + 1}, RevIt{begin}, closing | reverse(),
                         opening | reverse(), init_level, nestable);
        if (not res) return {};
    ...
    {
        auto res =
            find_closing(pos, end, opening, closing, init_level, nestable);
        if (not res) return {};
    ...
    {
        if (to_begin and first != last) first += (int)opening.length();
        if (to_end and first != last) last -= (int)closing.length();
    }
...
Optional<std::pair<Iterator, Iterator>> find_surrounding(
    const Container &container, Iterator pos, StringView opening,
    StringView closing, ObjectFlags flags, int init_level)
{
    return find_surrounding(begin(container), end(container), pos, opening,
                            closing, flags, init_level);
}
                             ...
                             ObjectFlags flags)
{
    ...
    auto res = find_surrounding(buffer, buffer.iterator_at(pos), opening,
                                closing, flags, level);
    if (not res) return selection;
    ...
    {
        if (auto res_parent =
                find_surrounding(buffer, buffer.iterator_at(pos), opening,
                                 closing, flags, level + 1))
                    ...
                    Codepoint c, int count, bool inclusive)
{
    ...
    do
    {
        ...
        if (end == buffer.end()) return selection;
    } while (--count > 0);
                            ...
                            Codepoint c, int count, bool inclusive)
{
    ...
    {
        --end;
        skip_while_reverse(end, buffer.begin(),
                           [c](Codepoint cur) { return cur != c; });
        if (end == buffer.begin()) return selection;
    } while (--count > 0);
                          ...
                          ObjectFlags flags)
{
    ...
    if (not(flags & ObjectFlags::ToEnd))
    {
        ...
        if (is_end_of_sentence(*prev_non_blank)) first = prev_non_blank;
    }
    ...
    if (flags & ObjectFlags::ToBegin)
    {
        ...
        while (first != buffer.begin())
        {
            char cur = *first;
            char prev = *(first - 1);
            if (not is_horizontal_blank(cur)) saw_non_blank = true;
                           ...
                           ObjectFlags flags)
{
    ...
    if (not(flags & ObjectFlags::ToEnd) and first.coord() > ByteCoord{0, 1} and
        *(first - 1) == '\n' and *(first - 2) == '\n')
        --first;
             ...
             (first + 1) != buffer.end() and *(first - 1) == '\n' and
             *first == '\n')
        ++first;
    ...
    if ((flags & ObjectFlags::ToBegin) and first != buffer.begin())
    {
        ...
        if (flags & ObjectFlags::ToEnd) last = first;
        while (first != buffer.begin())
        {
            char cur = *first;
            char prev = *(first - 1);
    ...
    if (flags & ObjectFlags::ToEnd)
    {
        if (last != buffer.end() and is_eol(*last)) ++last;
        while (last != buffer.end())
        {
                             ...
                             ObjectFlags flags)
{
    ...
    if (flags & ObjectFlags::ToBegin)
    {
        if (is_whitespace(*first))
        {
            ...
            if (not is_whitespace(*first)) ++first;
        }
    }
    if (flags & ObjectFlags::ToEnd)
    {
                          ...
                          int level, ObjectFlags flags)
{
    ...
    for (int lev = level; end != buffer.end(); ++end)
    {
        ...
        else if (c == Delimiter and lev == 0)
        {
            ...
            {
                while (end + 1 != buffer.end() and is_blank(*(end + 1))) ++end;
            }
            break;
        }
    }
                        ...
                        unsigned capture)
{
    ...
    if (capture > mark_count) throw runtime_error("invalid capture number");
    ...
    for (auto &sel : selections)
    {
        ...
        const auto flags = match_flags(
            is_bol(sel_beg.coord()), is_eol(buffer, sel_end.coord()),
            is_bow(buffer, sel_beg.coord()), is_eow(buffer, sel_end.coord()));
    ...
    if (result.empty()) throw runtime_error("nothing selected");
                      ...
                      unsigned capture)
{
    ...
    for (auto &sel : selections)
    {
        ...
        auto sel_end = utf8::next(buffer.iterator_at(sel.max()), buffer.end());
        const auto flags = match_flags(
            is_bol(begin.coord()), is_eol(buffer, sel_end.coord()),
            is_bow(buffer, begin.coord()), is_eow(buffer, sel_end.coord()));
...
UnitTest test_find_surrounding{[]() {
    ...
    {
        auto res =
            find_surrounding(s, s.begin() + 10, '{', '}',
                             ObjectFlags::ToBegin | ObjectFlags::ToEnd, 0);
        kak_assert(res and
                   StringView{res->first, res->second + 1} == "{ toi[] }");
    }
    {
        ...
        kak_assert(res and
                   StringView{res->first, res->second + 1} ==
                       "salut { toi[] }");
    }
    {
        auto res =
            find_surrounding(s, s.begin(), '[', ']',
                             ObjectFlags::ToBegin | ObjectFlags::ToEnd, 0);
        kak_assert(res and
                   StringView{res->first, res->second + 1} ==
                       "[salut { toi[] }]");
    ...
    {
        auto res = find_surrounding(s, s.begin() + 1, '[', ']',
    ...
    {
        auto res =
            find_surrounding(s, s.begin() + 1, '[', ']',
                             ObjectFlags::ToBegin | ObjectFlags::ToEnd, 0);
        kak_assert(res and StringView{res->first, res->second + 1} == "[]");
    }
    s = "[*][] hehe";
    {
        auto res = find_surrounding(s, s.begin() + 6, '[', ']',
                                    ObjectFlags::ToBegin, 0);
        kak_assert(not res);
    }
    ...
    {
        auto res =
            find_surrounding(s, s.begin() + 6, "begin", "end",
                             ObjectFlags::ToBegin | ObjectFlags::ToEnd, 0);
