...
Optional<Iterator> find_closing(Iterator pos,
                                Iterator end,
                                Container opening,
                                Container closing,
                                int init_level,
...
Optional<std::pair<Iterator, Iterator>> find_surrounding(Iterator begin,
                                                         Iterator end,
                                                         Iterator pos,
                                                         StringView opening,
                                                         StringView closing,
  ...
  if (to_begin) {
    ...
    auto res = find_closing(RevIt{pos + 1}, RevIt{begin}, closing | reverse(),
  ...
  auto last = pos;
  if (to_end) {
    auto res = find_closing(pos, end, opening, closing, init_level, nestable);
    if (not res)
      return {};
...
Selection select_surrounding(const Buffer& buffer,
  ...
  if (not nestable or flags & ObjectFlags::Inner) {
    if (auto res = find_surrounding(buffer, buffer.iterator_at(pos), opening,
                                    closing, flags, level))
  ...
  auto res = find_surrounding(buffer, buffer.iterator_at(pos), opening, closing,
                              flags, level);
  if (not res)
    return selection;
...
Selection select_to(const Buffer& buffer,
                    const Selection& selection,
                    Codepoint c,
                    int count,
                    bool inclusive) {
...
Selection select_to_reverse(const Buffer& buffer,
                            const Selection& selection,
                            Codepoint c,
                            int count,
                            bool inclusive) {
...
Selection select_number(const Buffer& buffer,
  ...
  if (flags & ObjectFlags::ToBegin) {
    ...
    if (not is_number(*first) and *first != '-' and first + 1 != buffer.end())
      ++first;
  }
    ...
    while (last != buffer.end()) {
      if (last != buffer.begin() and is_eol(*last) and is_eol(*(last - 1))) {
        if (not(flags & ObjectFlags::Inner))
          skip_while(last, buffer.end(), [](Codepoint c) { return is_eol(c); });
        break;
      }
      ++last;
    }
...
Selection select_indent(const Buffer& buffer,
  ...
  if (flags & ObjectFlags::Inner) {
    while (begin_line < end_line and is_only_whitespaces(buffer[begin_line]))
      ++begin_line;
...
UnitTest test_find_surrounding{[]() {
  ...
  }
  {
    ...
    kak_assert(res and
               StringView{res->first, res->second + 1} == "salut { toi[] }");
  }
  {
  ...
  {
    auto res =
        find_surrounding(s, s.begin() + 1, '[', ']', ObjectFlags::ToBegin, 0);
  ...
  {
    auto res =
        find_surrounding(s, s.begin() + 6, '[', ']', ObjectFlags::ToBegin, 0);
    kak_assert(not res);
  }
