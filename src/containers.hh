...
namespace Kakoune
{
template <typename Factory> struct ContainerView
{
    Factory factory;
};
...
template <typename Container> struct ReverseView
{
...
struct ReverseFactory
{
    ...
    ReverseView<RemoveReference<Container>> operator()(
...
template <typename Container> using ValueOf = typename Container::value_type;

template <typename Container, typename Filter> struct FilterView
{
                                    ...
                                    typename ContainerIt::value_type>
    {
        ...
        auto operator*() -> decltype(*std::declval<ContainerIt>())
        {
            return *m_it;
        }
        Iterator &operator++()
       ...
       private:
        void do_filter()
        {
            while (m_it != m_end and not m_view.m_filter(*m_it)) ++m_it;
        }
    ...
    {
        return {*this, m_container.begin(), m_container.end()};
    }
    Iterator end() const
    {
        return {*this, m_container.end(), m_container.end()};
    }
...
template <typename Filter> struct FilterFactory
{
    template <typename Container>
    FilterView<Container &, Filter> operator()(Container &container) const
    {
        return {container, std::move(m_filter)};
    }

    template <typename Container>
    FilterView<RemoveReference<Container>, Filter> operator()(
        Container &&container) const
    {
        return {std::move(container), std::move(m_filter)};
    }
...
inline ContainerView<FilterFactory<Filter>> filter(Filter f)
{
    return {{std::move(f)}};
}
...
template <typename Container, typename Transform> struct TransformView
{
    using ContainerIt = IteratorOf<Container>;
    ...
    struct Iterator
        : std::iterator<std::forward_iterator_tag,
                        typename std::remove_reference<
                            TransformedResult<ContainerIt, Transform>>::type>
    {
        ...
        auto operator*() -> TransformedResult<ContainerIt, Transform>
        {
            return m_view.m_transform(*m_it);
        }
        Iterator &operator++()
        {
            ++m_it;
...
template <typename Transform> struct TransformFactory
{
    template <typename Container>
    TransformView<Container &, Transform> operator()(Container &container) const
    {
        return {container, std::move(m_transform)};
    }

    template <typename Container>
    TransformView<RemoveReference<Container>, Transform> operator()(
        Container &&container) const
    {
        return {std::move(container), std::move(m_transform)};
    }
...
inline ContainerView<TransformFactory<Transform>> transform(Transform t)
{
    return {{std::move(t)}};
}
...
struct SplitView
{
    ...
    using ValueType = typename std::conditional<
        std::is_same<void, ValueTypeParam>::value,
    ...
    struct Iterator : std::iterator<std::forward_iterator_tag, ValueType>
    {
        ...
        {
            while (sep != end and *sep != separator) ++sep;
        }
    ...
    Iterator begin() const
    {
        return {m_container.begin(), m_container.end(), m_separator};
    }
    Iterator end() const
    {
        return {m_container.end(), m_container.end(), m_separator};
    }
...
template <typename ValueType, typename Separator> struct SplitViewFactory
{
    template <typename Container>
    SplitView<RemoveReference<Container>, Separator, ValueType> operator()(
        Container &&container) const
    {
        return {std::move(container), std::move(separator)};
    }

    template <typename Container>
    SplitView<Container &, Separator, ValueType> operator()(
        Container &container) const
    {
...
ContainerView<SplitViewFactory<ValueType, Separator>> split(Separator separator)
{
    return {{std::move(separator)}};
}

template <typename Container1, typename Container2> struct ConcatView
{
    ...
    struct Iterator : std::iterator<std::forward_iterator_tag, ValueType>
    {
        static_assert(std::is_convertible<typename ContainerIt1::value_type,
                                          ValueType>::value,
                      "");
        static_assert(std::is_convertible<typename ContainerIt2::value_type,
                                          ValueType>::value,
                      "");
        ...
        decltype(*std::declval<ContainerIt1>()) operator*()
        {
            return is2() ? *m_it2 : *m_it1;
        }
        Iterator &operator++()
        {
            if (is2())
    ...
    Iterator begin() const
    {
        return {m_container1.begin(), m_container1.end(), m_container2.begin()};
    }
    Iterator end() const
    {
        return {m_container1.end(), m_container1.end(), m_container2.end()};
    }
...
ConcatView<Container1, Container2> concatenated(Container1 &&container1,
