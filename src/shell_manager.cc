                  ...
                  ConstArrayView<String> kak_env, Func setup_child)
{
    ...
    for (char **envp = environ; *envp; ++envp) envptrs.push_back(*envp);
    for (auto &env : kak_env) envptrs.push_back(env.c_str());
    envptrs.push_back(nullptr);

    const char *shell = "/bin/sh";
    auto cmdlinezstr = cmdline.zstr();
    Vector<const char *> execparams = {shell, "-c", cmdlinezstr};
    if (not params.empty()) execparams.push_back(shell);
    for (auto &param : params) execparams.push_back(param.c_str());
                            ...
                            const ShellContext &shell_context)
{
    ...
    for (RegexIterator<const char *> it{cmdline.begin(), cmdline.end(), re},
         end;
         it != end; ++it)
    {
        ...
        auto match_name = [&](const String &s) {
            return s.length() > name.length() and prefix_match(s, name) and
                   s[name.length()] == '=';
        };
        if (contains_that(kak_env, match_name)) continue;

        auto var_it = shell_context.env_vars.find(name);
        try
        {
            const String &value =
                var_it != shell_context.env_vars.end()
                    ? var_it->value
                    : ShellManager::instance().get_val(name, context);
                                          ...
                                          const ShellContext &shell_context)
{
            ...
            (child_stdout.read_fd() != -1 or child_stderr.read_fd() != -1)))
    {
        EventManager::instance().handle_next_events(EventMode::Urgent,
                                                    &orig_mask);
        if (not terminated) terminated = waitpid(pid, &status, WNOHANG);
    }
    ...
    if (profile)
    {
        ...
        write_to_debug_buffer(format(
            "shell execution took {} ms (spawn: {}, wait: {})",
...
String ShellManager::get_val(StringView name, const Context &context) const
{
    auto env_var = std::find_if(
        m_env_vars.begin(), m_env_vars.end(), [name](const EnvVarDesc &desc) {
            return desc.prefix ? prefix_match(name, desc.str)
                               : name == desc.str;
        });
                                             ...
                                             ByteCount cursor_pos) const
{
    return complete(
        prefix, cursor_pos,
        m_env_vars | transform([](const EnvVarDesc &desc) -> const String & {
            return desc.str;
        }));
}
}
