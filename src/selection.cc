...
SelectionList::SelectionList(Buffer& buffer,
                             Vector<Selection> s,
                             size_t timestamp)
    : m_buffer(&buffer), m_selections(std::move(s)), m_timestamp(timestamp) {
  kak_assert(size() > 0);
...
Iterator merge_overlapping(Iterator begin,
                           Iterator end,
                           size_t& main,
...
const Buffer::Change* forward_sorted_until(const Buffer::Change* first,
  ...
  if (first != last) {
    ...
    while (++next != last) {
      const auto& ref =
          first->type == Buffer::Change::Insert ? first->end : first->begin;
      if (next->begin <= ref)
        return next;
      first = next;
    }
...
void update_forward(ConstArrayView<Buffer::Change> changes,
  ...
  kak_assert(
      std::is_sorted(selections.begin(), selections.end(), compare_selections));
}
...
Vector<Selection> compute_modified_ranges(Buffer& buffer, size_t timestamp) {
  ...
  while (change_it != changes.end()) {
    ...
    kak_assert(std::is_sorted(ranges.begin() + prev_size, ranges.end(),
                              compare_selections));
    std::inplace_merge(ranges.begin(), ranges.begin() + prev_size, ranges.end(),
                       compare_selections);
    ranges.erase(
        merge_overlapping(ranges.begin(), ranges.end(), dummy, overlaps),
        ranges.end());
  ...
  size_t dummy = 0;
  ranges.erase(merge_overlapping(ranges.begin(), ranges.end(), dummy, touches),
               ranges.end());
...
void SelectionList::update() {
  ...
  while (change_it != changes.end()) {
    ...
    m_selections.erase(merge_overlapping(m_selections.begin(),
                                         m_selections.end(), m_main, overlaps),
                       m_selections.end());
...
ByteCoord prepare_insert(Buffer& buffer,
                         const Selection& sel,
                         InsertMode mode) {
  switch (mode) {
...
void SelectionList::insert(ConstArrayView<String> strings,
                           InsertMode mode,
...
Selection selection_from_string(StringView desc) {
  ...
  if (comma == desc.end() or dot_anchor == comma or dot_cursor == desc.end())
    throw runtime_error(format(
        "'{}' does not follow <line>.<column>,<line>.<column> format", desc));
