...
void Selection::merge_with(const Selection &range)
{
    ...
    if (m_anchor < m_cursor) m_anchor = std::min(m_anchor, range.m_anchor);
    if (m_anchor > m_cursor) m_anchor = std::max(m_anchor, range.m_anchor);
}
...
{
    if (coord < begin) return coord;
    if (begin.line == coord.line) coord.column += end.column - begin.column;
                           ...
                           OverlapsFunc overlaps)
{
    ...
    for (size_t j = 1; j < size; ++j)
    {
        ...
        {
            ++i;
            if (i != j) begin[i] = std::move(begin[j]);
        }
    }
    return begin + i + 1;
}
...
struct ForwardChangesTracker
{
    ...
    {
        for (auto &change : buffer.changes_since(timestamp)) update(change);
    ...
    {
        if (coord < old_pos) return cur_pos;
                                            ...
                                            const Buffer::Change *last)
{
    if (first != last)
    {
        ...
        while (++next != last)
        {
            if (first->begin <= next->end) return next;
            first = next;
        }
    }
    return last;
                    ...
                    Vector<Selection> &selections)
{
    ...
    auto advance_while_relevant = [&](const ByteCoord &pos) mutable {
        while (change_it != changes.end() and
               changes_tracker.relevant(*change_it, pos))
                     ...
                     Vector<Selection> &selections)
{
    ...
    auto advance_while_relevant = [&](const ByteCoord &pos) mutable {
        while (change_it != change_end)
        {
            ...
            if (not changes_tracker.relevant(change, pos)) break;
...
Vector<Selection> compute_modified_ranges(Buffer &buffer, size_t timestamp)
{
    ...
    for (auto &sel : ranges)
    {
        if (buffer.is_end(sel.anchor())) sel.anchor() = buffer.back_coord();
        if (buffer.is_end(sel.cursor())) sel.cursor() = buffer.back_coord();
...
{
    if (m_timestamp == m_buffer->timestamp()) return;
    ...
    while (change_it != changes.end())
    {
        ...
        m_selections.erase(
            merge_overlapping(m_selections.begin(), m_selections.end(), m_main,
                              overlaps),
            m_selections.end());
...
ByteCoord prepare_insert(Buffer &buffer, const Selection &sel, InsertMode mode)
{
    switch (mode)
    {
        ...
        case InsertMode::Replace:
            return {};  // replace is handled specially, by calling
                        // Buffer::replace
...
void SelectionList::erase()
{
    ...
    for (auto &sel : m_selections)
    {
        if (sel.anchor() > back_coord) sel.anchor() = back_coord;
        if (sel.cursor() > back_coord) sel.cursor() = back_coord;
    }
...
{
    if (desc.empty()) throw runtime_error{"empty selection description"};
