...
template <>
struct WithBitOps<Attribute> : std::true_type {};
...
struct Face {
  ...
  constexpr Face(Color fg = Color::Default,
...
constexpr bool operator==(const Face& lhs, const Face& rhs) {
  return lhs.fg == rhs.fg and lhs.bg == rhs.bg and
...
constexpr Face merge_faces(const Face& base, const Face& face) {
  return face.attributes & Attribute::Exclusive
             ? face
             : Face{face.fg == Color::Default ? base.fg : face.fg,
